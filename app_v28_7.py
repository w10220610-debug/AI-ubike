...
(() => {
  ...
  function commandHelp(){
    speak("固定口令：賈維斯分析。賈維斯下一站。賈維斯抵達。要改數字一定說，賈維斯輸入上一般兩電，或賈維斯輸入下兩般一電。賈維斯完成。賈維斯跳過。賈維斯電池。賈維斯超低電。賈維斯重複。");
  }
  function handleTranscript(raw){
    ...
    if(explicitInput){
      ...
      if(emitAction("fill_actual", {station_name:station, ...actual, heard:transcript})) {
        setIndicator("正在套用上下車數量",true);
        speak(`${station}，${phrase}，已送出，請看畫面確認`);
      }
      return;
    }
    ...
    if(/^(?:清除輸入|清除输入|上下歸零|上下归零)$/.test(command)){
      ...
      if(emitAction("fill_actual",{station_name:station,...actual,heard:transcript})) {
        speak(`${station}，上下車數量已送出歸零，請看畫面確認`);
      }
      return;
    }
    ...
    if(/我到了|已抵達|已抵达|抵達|抵达|到目的地|到站了|我到站|^到了$/.test(command)){
      ...
      if(text(args.mode)==="candidate") {
        ...
        speak(`${station}，已抵達，正在鎖定場站`);
      } else speak(`${station}，已抵達`);
      return;
    }
    if(/有人去|有人處理|有人处理|換下一站|换下一站|改往其他|跳過|跳过|不要去/.test(command)){
      if(!contextReady()){speak(contextReply());return;}
      emitAction("skip_current",{station_name:currentStation(),reason:/有人/.test(command)?"有人已前往":"使用者語音改站",heard:transcript}); return;
    }
...
def get_jarvis_voice_component():
    ...
    component_dir = Path(tempfile.gettempdir()) / "taitung_jarvis_voice_component_v5"
    component_dir.mkdir(parents=True, exist_ok=True)
    index_path = component_dir / "index.html"
    ...
    _JARVIS_VOICE_COMPONENT = components.declare_component("taitung_jarvis_voice_v3", path=str(component_dir))
    return _JARVIS_VOICE_COMPONENT
