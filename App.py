...
def build_long_distance_status_dataframe(
    *,
    ...
    same_sheet_options = [
        (sheet_name, route)
        for sheet_name, route in options
        if sheet_name == selected_sheet and normalize_long_distance_zone(route) in selected_zone_set
    ]
    candidate_options = same_sheet_options or [
        (sheet_name, route)
        for sheet_name, route in options
        if normalize_long_distance_zone(route) in selected_zone_set
    ]

    # 每個 D2／D3 區域只取同一個最接近目前配置版本的來源，避免重複場站。
    chosen_by_zone: dict[str, tuple[str, str]] = {}
    for sheet_name, route in candidate_options:
        zone = normalize_long_distance_zone(route)
        if zone and zone not in chosen_by_zone:
            chosen_by_zone[zone] = (sheet_name, route)
...
def adjust_candidates_for_trip_mode(
    ...
    *,
    ...
    """來回將返程完整計入；單趟指定終點則將往終點的距離納入後續方向性。"""
    adjusted: list[dict] = []
    ...
    for original in candidates:
        ...
        # 單趟指定終點只採 35% 方向性權重，避免第一站過度被終點綁死；來回則完整計入返程。
        endpoint_weight = 1.0 if trip_mode == "來回" else (0.35 if endpoint_valid else 0.0)
...
def build_long_distance_route_preview(
    ...
    *,
    ...
    """用逐站重算方式建立可預覽路線；每站完成後正式系統仍會依即時資料再算一次。"""
    ...
    for stop_index in range(max(1, max_stops)):
        ...
        chosen = candidates[0]
        if stop_index == 0 and forced_first_station:
            forced = next(
        ...
        chosen = dict(chosen)
        chosen["preview_order"] = stop_index + 1
        ...
        simulated_location = {
            ...
            "updated_at": now_timestamp + stop_index + 1,
...
def render_long_distance_route_preview(
    ...
    *,
    ...
    st.caption("此為目前即時資料的路線預覽；每完成一站或資料明顯變動後，後續順序會重新計算。")
...
def render_smart_dispatch(
    *,
    ...
    """逐站詢問、載量限制、動態重算；長途頁可再加入單趟／來回與手動指定下一站。"""
...
def render_long_distance_route_page(
    *,
    ...
    active_trip_key = (
        f"smart_dispatch::long_distance::{active_base['token']}::{long_context_key}::active_trip"
    )
    trip_locked = isinstance(st.session_state.get(active_trip_key), dict)

    with st.expander("本次長途任務設定" + ("（前往中已鎖定）" if trip_locked else ""), expanded=not trip_locked):
...
with st.expander("YouBike 即時車數", expanded=False):
