...
APP_VERSION = "v26.2"
APP_VERSION_NAME = "懸浮搜尋整合與電池行政區統整版"
...
def render_floating_battery_query(
    ...
    (() => {
      ...
      style.textContent = `
        ...
        #ubike-battery-page .battery-district-summary .battery-metric {
          padding: 10px;
          background: #f8fbf6;
        }
        #ubike-battery-page .battery-district-summary .battery-metric-value { font-size: 21px; }
      ...
      page.innerHTML = `
        ...
        <main class="battery-main">
          ...
          <section class="battery-district-summary" hidden>
            <div class="battery-district-summary-title">按行政區統整電池數量</div>
      ...
      function buildDisplayGroups(threshold) {
        // 清單只保留確實有低電量車輛的場站；零顆與未配對場站不再顯示。
        const decorated = lastResults
          .filter(result => result.matched && lowBikesFor(result, threshold).length > 0)
          .map(result => ({
            result,
      ...
      function renderResults() {
        ...
        for (const [district, total] of sortedDistrictTotals) {
          const metric = doc.createElement("div");
          metric.className = "battery-metric";
          appendTextElement(metric, "div", "battery-metric-label", district);
...
if page_mode == "智慧調度":
    smart_dispatch_search_source = render_long_distance_route_page(
        active_base=active_base,
        options=configuration_options,
        selected_sheet=selected_sheet,
        selected_shift=selected_shift,
    ...
    if not isinstance(smart_dispatch_search_source, pd.DataFrame) or smart_dispatch_search_source.empty:
        smart_dispatch_search_source = base_df
    render_floating_station_search(
        build_result_with_recognition_errors(smart_dispatch_search_source),
        mobile_input_mode,
        page_mode=page_mode,
    )
...
# 一般分析的懸浮搜尋只讀取目前排序完成、實際顯示的分析結果。
render_floating_station_search(result_df, mobile_input_mode, page_mode=page_mode)
render_floating_battery_query(
    build_battery_route_station_map(base_df),
    mobile_input_mode,
)
