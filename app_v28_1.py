...
(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent=`
      #jarvis-secret-trigger{touch-action:manipulation;user-select:none;-webkit-user-select:none;cursor:default}
  ...
  function attachTrigger(){
    ...
    trigger.onclick=(event)=>{
      event.preventDefault();
      clickCount += 1;
      if(clickResetTimer) win.clearTimeout(clickResetTimer);
      if(clickCount >= 5){
        clickCount=0;
      ...
      clickResetTimer=win.setTimeout(()=>{clickCount=0;clickResetTimer=null;},2200);
    };
...
def get_jarvis_voice_component():
    """建立隱藏式賈維斯語音元件；快速連點「測試版」標籤五下才會啟用。"""
