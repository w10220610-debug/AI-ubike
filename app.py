    ...
    *,
    ...
    if visible_candidates:
        ...
        with st.expander(panel_title, expanded=False):
            ...
            # 移除多行 HTML 的共同縮排，避免 Streamlit Markdown 將候選卡片誤判為程式碼區塊。
            candidate_panel_html = dedent(
                f"""
                <div class="dispatch-candidate-grid">
                {''.join(candidate_cards)}
                </div>
                """
            ).strip()
            st.markdown(candidate_panel_html, unsafe_allow_html=True)
