...
(() => {
  ...
  async function runSync() {
    ...
    setStatus("資料會由你目前這支手機／瀏覽器直接取得，不經 Streamlit 雲端主機。", false);
...
with st.expander("🌐 YouBike 官網即時車數（免 TDX 金鑰）", expanded=True):
    st.caption(
        "這個版本不再由 Streamlit 雲端主機直接連 YouBike，改成由你目前使用的手機／"
        "瀏覽器向 YouBike 官網取得資料，再把 2.0 與 2.0E 車數傳回分析系統。"
    )
    st.success(
        "✅ 不需要 TDX、Client ID、Client Secret 或 Streamlit Secrets；也能避開"
        " Streamlit Cloud 主機被官網回傳 HTTP 503 的問題。"
    )
    ...
    try:
        ...
        browser_payload = browser_sync_component(
            ...
            batch_size=10,
            button_label="🔄 取得 YouBike 官網即時車數並寫入現況",
                ...
                with st.spinner("正在配對臺東場站名稱並寫入 2.0／2.0E 現況……"):
                    ...
                    if live_summary["matched_count"] <= 0:
                        ...
                        st.success(
                            ...
                            f"{live_summary['total_count']} 個 Excel 場站{fetch_text}｜系統取得時間："
                            f"{live_payload['fetched_at']}{official_time_text}"
                        )
                        if missing_count:
                            st.warning(
                                f"官網本次仍有 {missing_count} 個站號未回傳即時資料；"
                                "系統已自動補查一次，未取得者不會用 0 覆蓋。"
                            )
    ...
    st.caption(
        "資料仍來自 YouBike 微笑單車官網公開 JSON 接口；差別只在於請求改由你的瀏覽器發出，"
        "不再經過容易被擋的 Streamlit Cloud 伺服器 IP。官方資料可能有約 1 分鐘延遲；"
        "場站非正常服務、車種明細不完整或名稱配對不確定時，系統不會自動覆蓋。"
    )
