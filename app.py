...
def get_local_ocr_engine():
    """載入 RapidOCR，並把雲端缺少套件或 Linux 元件的原因完整顯示。"""
    import_errors: list[str] = []

    try:
        import onnxruntime  # noqa: F401
    except Exception as exc:
        import_errors.append(f"onnxruntime 載入失敗：{type(exc).__name__}: {exc}")

    rapid_ocr_class = None
    try:
        from rapidocr import RapidOCR as RapidOCRClass

        rapid_ocr_class = RapidOCRClass
    except Exception as exc:
        import_errors.append(f"rapidocr 載入失敗：{type(exc).__name__}: {exc}")

        # 相容舊專案曾經使用的套件名稱；新環境通常不會走到這裡。
        try:
            from rapidocr_onnxruntime import RapidOCR as LegacyRapidOCRClass

            rapid_ocr_class = LegacyRapidOCRClass
        except Exception as legacy_exc:
            import_errors.append(
                "rapidocr_onnxruntime 載入失敗："
    ...
    try:
        ...
        raise RuntimeError(
            f"{prefix}RapidOCR 初始化失敗：{type(exc).__name__}: {exc}"
        ) from exc
...
with st.expander("📷 多張照片辨識場站現況", expanded=False):
    ...
    if st.button(
        ...
        try:
            ...
            st.error("OCR 啟動或照片辨識失敗")
            st.code(f"{type(exc).__name__}: {exc}", language="text")
            with st.expander("查看完整錯誤內容", expanded=False):
                st.code(traceback.format_exc(), language="text")
            st.caption(
                "雲端版不需要在手機或電腦執行安裝OCR.bat；"
                "套件必須由 GitHub 同資料夾內的 requirements.txt 安裝。"
            )
