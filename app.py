...
def apply_youbike_updates_to_dataframe(
    ...
    """將 YouBike 官網即時車數、空位、總柱與服務狀態寫入目前配置。"""
...
def merge_current_status(full_df: pd.DataFrame, edited_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return _apply_status_lookup(full_df, edited_df)
...
def render_analysis_result_table(region_df: pd.DataFrame) -> None:
    ...
    for row_index, row in display_df.iterrows():
        distance = pd.to_numeric(pd.Series([row["距離目前位置 (km)"]]), errors="coerce").iloc[0]
        minutes = pd.to_numeric(pd.Series([row["預估行車時間 (分)"]]), errors="coerce").iloc[0]
        rows.append((
            int(row_index), str(row["場站名稱"]), str(row.get("路線區域") or ""),
            optional_count_text(row["2.0 現況"]), safe_nonnegative_int(row["2.0 標準"]),
            str(row["2.0 缺／多幾台"]),
            optional_count_text(row["2.0E 現況"]), safe_nonnegative_int(row["2.0E 標準"]),
            str(row["2.0E 缺／多幾台"]),
            optional_count_text(row["總車數"]), optional_count_text(row["空位數"]),
...
def render_floating_station_search(result_df: pd.DataFrame, mobile_mode: bool) -> None:
    ...
    floating_fingerprint = hashlib.sha1(
        f"{display_mode}|{station_payload}".encode("utf-8")
    ).hexdigest()
...
def render_floating_battery_query(
    ...
    battery_fingerprint = json.dumps(
        hashlib.sha1(f"{display_mode}|{route_payload}".encode("utf-8")).hexdigest()
    )
    component_html = r"""
    <script>
    (() => {
...
def build_battery_route_station_map(status_df: pd.DataFrame) -> dict[str, list[dict]]:
    ...
    for _, row in status_df.iterrows():
        route = str(row.get("路線區域", "") or "").strip().upper()
...
def notify_station_alert_changes(
    ...
    *,
    ...
    alerts = build_station_alert_records(status_df)
...
def render_station_alert_summary(
    ...
    *,
    ...
    alerts = add_road_metrics_to_alerts(
        build_station_alert_records(status_df),
        station_locations=station_locations,
        current_location=current_location,
    )
    if not alerts:
        return []
    urgent_count = sum(1 for item in alerts if safe_nonnegative_int(item.get("severity")) >= 2)
    warning_count = len(alerts) - urgent_count
    with st.expander(
        ...
        if len(alerts) > 15:
            st.caption(f"另有 {len(alerts) - 15} 站警示，可使用全場站搜尋查看。")
    return alerts
...
def render_smart_dispatch(
    *,
    ...
    # 全場站搜尋不受前 15 名候選限制；搜尋只改變目前選擇，最後仍需按「前往此站」才鎖定。
    with st.expander("🔎 搜尋全部場站", expanded=False):
        search_text = st.text_input(
            "輸入場站名稱",
            placeholder="可輸入部分名稱，例如：轉運站、森林公園、池上……",
            key=f"{dispatch_prefix}::station_search_text",
        ).strip()
        all_station_names = sorted(
            dispatch_df["場站名稱"].dropna().astype(str).drop_duplicates().tolist()
        )
        if search_text:
            normalized_query = normalize_station_key(search_text)
            matched_names = [
                name for name in all_station_names
                if normalized_query in normalize_station_key(name)
            ]
            if not matched_names:
                st.warning("找不到符合的場站名稱。")
            else:
                searched_station = st.selectbox(
                    f"搜尋結果（{len(matched_names)} 站）",
                    matched_names,
                    key=(
                        f"{dispatch_prefix}::station_search_result::"
                        f"{hashlib.sha1(normalized_query.encode('utf-8')).hexdigest()[:10]}"
                    ),
                )
                allowed_route_zone = (
                    active_loop_phase
                ...
                inspection = inspect_dispatch_station(
                    ...
                    cooldowns=cooldowns,
                    current_round=current_round,
                    allowed_route_zone=allowed_route_zone,
                )
                render_station_inspection_card(inspection)
                searched_candidate = next(
                    (
                        candidate for candidate in candidates
                        if str(candidate.get("station_name")) == str(searched_station)
                    ),
                    None,
                )
                if st.button(
                    ...
                    type="primary",
                    use_container_width=True,
                    disabled=not bool(inspection.get("processable") and searched_candidate),
                    key=f"{dispatch_prefix}::use_searched_station::{normalize_station_key(searched_station)}",
                    help=(
                        "只套用為目前選擇；下方仍需按『前往此站』才會正式鎖定。"
                        if inspection.get("processable") and searched_candidate
                        else str(inspection.get("reason") or "目前無法設為下一站。")
                    ),
                ):
                    if str(searched_station) == str(candidates[0]["station_name"]):
                        st.session_state.pop(manual_station_key, None)
                    else:
                        st.session_state[manual_station_key] = str(searched_station)
                    persist_runtime_state(active_base["token"])
                    rerun_app()
        else:
            st.caption(f"可搜尋目前配置中的全部 {len(all_station_names)} 個場站，不限缺多車或 AI 排名。")
...
def cached_load_workbook(source: bytes | str) -> dict[str, pd.DataFrame]:
    """只開啟 Excel 一次，並排除 hidden／veryHidden 工作表。"""
...
result_df, road_metrics_available, road_metrics_message = attach_analysis_road_metrics(
    result_df,
    station_locations=combined_station_locations,
    current_location=shared_location,
)
...
source_region_groups = {
    str(region): region_df.copy()
    for region, region_df in edited_df.groupby(edited_df["行政區"].astype(str), sort=False)
}
result_region_groups = {
    str(region): region_df.copy()
    for region, region_df in result_df.groupby(result_df["行政區"].astype(str), sort=False)
}
for region, region_source_df in source_region_groups.items():
    region_result_df = result_region_groups.get(region, result_df.iloc[0:0].copy())
    zone_values = []
    ...
    if export_tools_open:
        with st.container(border=True):
            ...
            render_new_window_download_panel(
                csv_data=csv_data,
                csv_filename=(
                    f"{selected_configuration_type}_D1_D2_D3_{selected_shift}_調度分析_彩色標記.csv"
                ),
                excel_data=excel_data,
                excel_filename=(
                    f"{selected_configuration_type}_D1_D2_D3_{selected_shift}_調度分析_彩色.xlsx"
                ),
            )
