...
with st.expander("📷 多張照片直接更新現況", expanded=False):
    st.caption(
        "照片格式請保持與站點列表相同。系統只讀取『在站』下方的 2.0、2.0E；"
        "辨識成功的場站會直接寫入現況；無法辨識的資料會顯示『識別錯誤』。"
    )
    ...
    uploaded_station_photos = st.file_uploader(
        ...
        help="可一次選取多張照片；請保留欄位標題，並避免反光、模糊或裁掉場站名稱與在站數字。",
    )
    ...
    if st.button(
        ...
        try:
            with st.spinner(f"正在辨識 {len(uploaded_station_photos)} 張照片並更新現況……"):
                analysis_result = analyze_station_photos(uploaded_station_photos, base_df)
