...
TDX_TAITUNG_STATION_URL = (
    "https://tdx.transportdata.tw/api/basic/v2/Bike/Station/City/"
    "TaitungCounty?%24top=1000&%24format=JSON"
)
TDX_TAITUNG_AVAILABILITY_URL = (
    "https://tdx.transportdata.tw/api/basic/v2/Bike/Availability/City/"
    "TaitungCounty?%24top=1000&%24format=JSON"
)
TDX_REQUEST_TIMEOUT_SECONDS = 25
TDX_MATCH_THRESHOLD = 0.82
...
def read_tdx_credentials_from_config() -> tuple[str, str]:
    """優先讀取 Streamlit secrets，其次讀取環境變數；不把金鑰寫進程式碼。"""
    client_id = ""
    client_secret = ""

    try:
        tdx_section = st.secrets.get("tdx", {})
        if hasattr(tdx_section, "get"):
            client_id = str(tdx_section.get("client_id", "") or "").strip()
            client_secret = str(tdx_section.get("client_secret", "") or "").strip()

        if not client_id:
            client_id = str(st.secrets.get("TDX_CLIENT_ID", "") or "").strip()
        if not client_secret:
            client_secret = str(st.secrets.get("TDX_CLIENT_SECRET", "") or "").strip()
    except Exception:
        # 尚未建立 secrets.toml 時，Streamlit 可能拋出檔案不存在例外。
        pass

    if not client_id:
        client_id = str(os.getenv("TDX_CLIENT_ID", "") or "").strip()
    if not client_secret:
        client_secret = str(os.getenv("TDX_CLIENT_SECRET", "") or "").strip()
...
def _tdx_http_json(
    ...
    *,
    ...
    """以標準函式庫呼叫 TDX，避免額外增加 requests 套件。"""
    request_headers = {
        "Accept": "application/json",
        "User-Agent": "Taitung-YouBike-Dispatch/1.0",
    }
    if headers:
    ...
    try:
        with urlopen(request, timeout=TDX_REQUEST_TIMEOUT_SECONDS) as response:
        ...
        if exc.code in (401, 403):
            raise TDXDataError(
                "TDX 驗證失敗，請確認 Client ID、Client Secret 與服務訂閱狀態。"
            ) from exc
        if exc.code == 429:
            raise TDXDataError("TDX 呼叫次數過多，請稍後再重新取得。") from exc
        raise TDXDataError(
            f"TDX 回傳 HTTP {exc.code}。{detail or '請稍後再試。'}"
        ) from exc
    except URLError as exc:
        raise TDXDataError(f"無法連線至 TDX：{getattr(exc, 'reason', exc)}") from exc
    except TimeoutError as exc:
        raise TDXDataError("連線 TDX 逾時，請檢查網路後重試。") from exc

    try:
        return json.loads(raw_body)
    except json.JSONDecodeError as exc:
        raise TDXDataError("TDX 回傳內容不是有效的 JSON。") from exc


@st.cache_data(show_spinner=False, ttl=82800, max_entries=6)
def get_tdx_access_token(client_id: str, client_secret: str) -> str:
    """取得並快取 TDX Access Token，避免每次同步都重新驗證。"""
    payload = _tdx_http_json(
        TDX_TOKEN_URL,
        method="POST",
        form_data={
    ...
    if not access_token:
        raise TDXDataError("TDX 未回傳 Access Token，請重新確認 API 金鑰。")
    return access_token


def _extract_tdx_items(payload) -> list[dict]:
    """相容 TDX 直接陣列與包在 value 欄位內的回傳格式。"""
...
def _tdx_station_name(record: dict) -> str:
    station_name = record.get("StationName", {})
    if isinstance(station_name, dict):
        return str(
            station_name.get("Zh_tw")
            or station_name.get("Zh_TW")
            or station_name.get("En")
            or ""
        ).strip()
    return str(station_name or "").strip()


def normalize_tdx_station_key(value) -> str:
    """正規化 Excel 與 TDX 場站名稱，處理臺／台及 YouBike 前綴差異。"""
...
def _tdx_station_similarity(excel_name: str, api_name: str) -> float:
    excel_key = normalize_tdx_station_key(excel_name)
    api_key = normalize_tdx_station_key(api_name)
...
def fetch_tdx_taitung_bike_data(
    client_id: str,
    client_secret: str,
    refresh_bucket: int,
) -> dict:
    """取得臺東縣公共自行車站名與即時可借 2.0／2.0E 數量。"""
    del refresh_bucket  # 僅用於讓快取每 30 秒自然換批。
    token = get_tdx_access_token(client_id, client_secret)
    headers = {"Authorization": f"Bearer {token}"}

    station_payload = _tdx_http_json(TDX_TAITUNG_STATION_URL, headers=headers)
    availability_payload = _tdx_http_json(TDX_TAITUNG_AVAILABILITY_URL, headers=headers)
    station_items = _extract_tdx_items(station_payload)
    availability_items = _extract_tdx_items(availability_payload)
    ...
    for station in station_items:
        ...
        station_name = _tdx_station_name(station)
        if station_uid and station_name:
            station_names[station_uid] = station_name
            station_ids[station_uid] = station_id

    records: list[dict] = []
    source_times: list[str] = []
    for availability in availability_items:
        station_uid = str(availability.get("StationUID", "") or "").strip()
        station_name = station_names.get(station_uid, "")
        if not station_uid or not station_name:
            continue

        detail = availability.get("AvailableRentBikesDetail", {})
        if not isinstance(detail, dict):
            detail = {}

        general_bikes = normalize_current_status(detail.get("GeneralBikes"))
        electric_bikes = normalize_current_status(detail.get("ElectricBikes"))
        service_status = safe_nonnegative_int(availability.get("ServiceStatus"))
        source_update_time = str(availability.get("SrcUpdateTime", "") or "").strip()
        platform_update_time = str(availability.get("UpdateTime", "") or "").strip()
        ...
        records.append(
            {
                ...
                "station_id": station_ids.get(station_uid, ""),
                "station_name": station_name,
                "station_key": normalize_tdx_station_key(station_name),
    ...
    if not records:
        raise TDXDataError("TDX 沒有回傳可用的臺東場站即時資料。")
...
def match_tdx_station(excel_name: str, live_records: list[dict]) -> tuple[dict | None, float, bool]:
    """配對 Excel 場站與 TDX 場站；名稱過於接近時不自動寫入，避免配錯站。"""
    excel_key = normalize_tdx_station_key(excel_name)
    if not excel_key:
    ...
    ranked = sorted(
        (
            (_tdx_station_similarity(excel_name, record.get("station_name", "")), record)
    ...
    if best_score < TDX_MATCH_THRESHOLD:
...
def apply_tdx_updates_to_dataframe(
    base_df: pd.DataFrame,
    live_records: list[dict],
) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    """將 TDX 即時車數寫入目前配置；只寫入唯一且服務正常的安全配對。"""
    updated_df = base_df.copy()
    report_rows: list[dict] = []
    used_station_uids: set[str] = set()
    matched_count = 0
    skipped_count = 0
    unmatched_count = 0
    ...
    for row_index, row in updated_df.iterrows():
        ...
        matched_record, score, ambiguous = match_tdx_station(excel_name, live_records)
        ...
        station_uid = str(matched_record.get("station_uid", "") or "")
        if station_uid in used_station_uids:
            unmatched_count += 1
            report_rows.append(
                {
                    "Excel 場站": excel_name,
                    "TDX 場站": matched_record.get("station_name", ""),
                    "2.0": pd.NA,
                    "2.0E": pd.NA,
                    "結果": "同一 TDX 場站已配對，未重複寫入",
                    "相似度": round(score, 3),
                }
            )
            continue
        ...
        if service_status != 1 or bike_count is None or ebike_count is None:
            skipped_count += 1
            reason = "場站目前非正常服務" if service_status != 1 else "TDX 車種明細不完整"
            report_rows.append(
                {
                    "Excel 場站": excel_name,
                    "TDX 場站": matched_record.get("station_name", ""),
        ...
        used_station_uids.add(station_uid)
...
with st.expander("🌐 官方即時車數（TDX）", expanded=True):
    st.caption(
        "按下同步後，系統會讀取臺東縣各場站目前可借的 YouBike 2.0 與 2.0E，"
        "只把名稱安全配對成功的資料寫入目前選擇的分區與班別。"
    )

    configured_client_id, configured_client_secret = read_tdx_credentials_from_config()
    if configured_client_id and configured_client_secret:
        tdx_client_id = configured_client_id
        tdx_client_secret = configured_client_secret
        st.success("✅ 已從 Streamlit Secrets／環境變數讀取 TDX API 金鑰。")
    else:
        st.warning(
            "尚未設定 TDX API 金鑰。可先在下方暫時輸入測試；正式部署請放進 Streamlit Secrets，"
            "不要把 Client Secret 上傳到 GitHub。"
        )
        credential_col_1, credential_col_2 = st.columns(2)
        with credential_col_1:
            tdx_client_id = st.text_input(
                "TDX Client ID",
                value="",
                type="password",
                key="tdx_runtime_client_id",
            ).strip()
        with credential_col_2:
            tdx_client_secret = st.text_input(
                "TDX Client Secret",
                value="",
                type="password",
                key="tdx_runtime_client_secret",
            ).strip()

        with st.popover("查看 Streamlit Secrets 設定格式"):
            st.code(
                '[tdx]\nclient_id = "你的 Client ID"\nclient_secret = "你的 Client Secret"',
                language="toml",
            )
            st.caption("Streamlit Cloud：App settings → Secrets，貼上後儲存並重新啟動。")

    previous_tdx_meta = status_cache.get("metadata", {}).get(current_context_key, {})
    if isinstance(previous_tdx_meta, dict) and previous_tdx_meta.get("fetched_at"):
        st.info(
            f"此分區上次 TDX 同步：{previous_tdx_meta['fetched_at']}｜"
            f"成功寫入 {safe_nonnegative_int(previous_tdx_meta.get('matched_count'))} 個場站"
        )

    sync_tdx_now = st.button(
        "🔄 取得官方即時車數並寫入現況",
        type="primary",
        use_container_width=True,
        disabled=not (tdx_client_id and tdx_client_secret),
        help="為避免濫用 API，同一批資料最多快取約 30 秒。",
        key=f"sync_tdx::{current_context_key}",
    )

    if sync_tdx_now:
        try:
            with st.spinner("正在讀取 TDX 臺東場站即時車數並進行名稱配對……"):
                live_payload = fetch_tdx_taitung_bike_data(
                    tdx_client_id,
                    tdx_client_secret,
                    int(time.time() // 30),
                )
                tdx_updated_df, tdx_report_df, tdx_summary = apply_tdx_updates_to_dataframe(
                    base_df,
                    live_payload["records"],
                )

                if tdx_summary["matched_count"] <= 0:
                    st.error("沒有任何場站通過安全配對，因此未修改現況資料。")
                else:
                    base_df = tdx_updated_df
                    ...
                    status_cache.setdefault("metadata", {})[current_context_key] = {
                        ...
                        "matched_count": tdx_summary["matched_count"],
                        "skipped_count": tdx_summary["skipped_count"],
                        "unmatched_count": tdx_summary["unmatched_count"],
                    }
                    save_cached_status(
                        active_base["token"],
                        active_base["expires_at"],
                    ...
                    clear_editor_session_state(active_base["token"])
                    st.success(
                        f"✅ TDX 同步完成：已寫入 {tdx_summary['matched_count']}／"
                        f"{tdx_summary['total_count']} 個場站。資料取得時間："
                        f"{live_payload['fetched_at']}"
                    )

                if not tdx_report_df.empty:
                    problem_df = tdx_report_df[tdx_report_df["結果"] != "已寫入"]
                    with st.expander(
                        f"查看配對明細（未寫入 {len(problem_df)} 筆）",
                        expanded=not problem_df.empty,
                    ):
                        report_height = min(520, max(110, 42 + len(tdx_report_df) * 35))
                        st.dataframe(
                            tdx_report_df,
                            hide_index=True,
                            use_container_width=True,
            ...
            st.error(f"TDX 同步失敗：{exc}")
        except Exception as exc:
            st.error(f"TDX 同步發生未預期錯誤：{exc}")

    st.caption(
        "資料來源：交通部 TDX 運輸資料流通服務。即時資料可能有更新延遲；"
...
st.caption(
    "資料來源標示：交通部 TDX 運輸資料流通服務（官方即時同步功能）。"
