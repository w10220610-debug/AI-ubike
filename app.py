...
(() => {
  ...
  function scheduleAutoSync() {
    ...
    const seconds = Math.max(5, Math.min(60, Number(args.auto_refresh_seconds || 5)));
...
def build_dispatch_plan_for_station(
    ...
    *,
    ...
    # 空間不足時，優先載走整體缺口較大的車種；仍以本站總調度量最大化為第一目標。
...
def dispatch_action_text(plan: dict) -> str:
    ...
    if safe_nonnegative_int(plan.get("unload_bike")):
        parts.append(f"卸 2.0 × {safe_nonnegative_int(plan['unload_bike'])}")
    if safe_nonnegative_int(plan.get("unload_ebike")):
        parts.append(f"卸 2.0E × {safe_nonnegative_int(plan['unload_ebike'])}")
    if safe_nonnegative_int(plan.get("pickup_bike")):
        parts.append(f"載 2.0 × {safe_nonnegative_int(plan['pickup_bike'])}")
    if safe_nonnegative_int(plan.get("pickup_ebike")):
        parts.append(f"載 2.0E × {safe_nonnegative_int(plan['pickup_ebike'])}")
    return "｜".join(parts) if parts else "無可行調度"


def render_dispatch_plan_card(plan: dict, *, title: str) -> None:
    """呈現單一推薦場站的距離、時間、調度量與作業後載量。"""
    multiplier_text = ""
    if float(plan.get("reason_multiplier") or 1.0) < 0.999:
        multiplier_text = (
            f"<div class=\"dispatch-plan-note\">曾因「{html.escape(str(plan.get('last_rejection_reason') or '其他原因'))}」"
            f"跳過，本次效益已做現場原因修正。</div>"
        )
    st.markdown(
        f"""
        <section class="dispatch-plan-card">
          ...
          <div class="dispatch-plan-grid">
            <div><span>估計道路距離</span><strong>{float(plan['estimated_distance_km']):.1f} km</strong></div>
            <div><span>估計行車時間</span><strong>{float(plan['estimated_drive_minutes']):.0f} 分</strong></div>
            <div><span>本站可調度</span><strong>{safe_nonnegative_int(plan['dispatch_count'])} 台</strong></div>
            <div><span>綜合效益</span><strong>{float(plan['score']):.2f} 台／分</strong></div>
          </div>
          <div class="dispatch-plan-action">{html.escape(dispatch_action_text(plan))}</div>
          <div class="dispatch-plan-after">完成後貨車：2.0＝{safe_nonnegative_int(plan['truck_after_bike'])} 台｜2.0E＝{safe_nonnegative_int(plan['truck_after_ebike'])} 台</div>
          {multiplier_text}
        </section>
        """,
        unsafe_allow_html=True,
...
def render_smart_dispatch(
    *,
    ...
    st.markdown(
        ...
        <style>
        .dispatch-plan-card {
            border: 1px solid rgba(22,119,255,.24); border-radius: 18px;
            padding: 1rem; margin: .5rem 0 .8rem;
            background: linear-gradient(145deg, rgba(22,119,255,.08), rgba(16,185,129,.06));
            box-shadow: 0 10px 30px rgba(15,23,42,.07);
        }
        .dispatch-plan-kicker {font-size:.74rem; font-weight:800; letter-spacing:.08em; color:#1677ff;}
        .dispatch-plan-title {font-size:1.45rem; font-weight:850; margin-top:.15rem;}
        .dispatch-plan-region {font-size:.78rem; opacity:.72; margin-bottom:.75rem;}
        .dispatch-plan-grid {display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:.45rem;}
        .dispatch-plan-grid div {padding:.62rem .5rem; border-radius:12px; background:rgba(255,255,255,.72); text-align:center;}
        .dispatch-plan-grid span {display:block; font-size:.68rem; opacity:.68;}
        .dispatch-plan-grid strong {display:block; font-size:1rem; margin-top:.18rem;}
        .dispatch-plan-action {margin-top:.7rem; padding:.68rem .75rem; border-radius:12px; font-weight:800; background:rgba(22,119,255,.11);}
        .dispatch-plan-after,.dispatch-plan-note {margin-top:.45rem; font-size:.78rem; line-height:1.5; opacity:.82;}
        @media (max-width: 700px) {
          .dispatch-plan-grid {grid-template-columns:repeat(2,minmax(0,1fr));}
          .dispatch-plan-card {padding:.82rem;}
        }
        </style>
        """,
        unsafe_allow_html=True,
    ...
    with control_col_1:
        max_capacity = int(st.number_input(
            ...
            value=14,
            step=1,
            key=f"{dispatch_prefix}::max_capacity",
        ))
    with control_col_2:
        truck_bike = int(st.number_input(
            ...
            value=0,
            step=1,
            key=f"{dispatch_prefix}::truck_bike",
        ))
    with control_col_3:
        truck_ebike = int(st.number_input(
            ...
            value=0,
            step=1,
            key=f"{dispatch_prefix}::truck_ebike",
        ))
    ...
    if total_on_truck > max_capacity:
        ...
        return
    st.caption(f"目前載量：{total_on_truck}／{max_capacity} 台｜剩餘空位：{max_capacity - total_on_truck} 台")
    ...
    if cooldowns:
        cooldown_lines = []
        for station_name, data in sorted(cooldowns.items(), key=lambda item: float(item[1].get("until") or 0)):
        ...
        with st.expander(f"冷卻中的場站（{len(cooldowns)}）", expanded=False):
            st.write("  \n".join(cooldown_lines))
            if st.button("清除全部冷卻", key=f"{dispatch_prefix}::clear_cooldowns", use_container_width=True):
        ...
        with st.expander(f"本班次調度紀錄（{len(history)}）", expanded=False):
            ...
            for event in reversed(history[-30:]):
                ...
                if event.get("action") == "rejected":
                    ...
                    detail = (
                        f"卸2.0 {safe_nonnegative_int(event.get('unload_bike'))}、"
                        f"載2.0 {safe_nonnegative_int(event.get('pickup_bike'))}、"
                        f"卸2.0E {safe_nonnegative_int(event.get('unload_ebike'))}、"
                        f"載2.0E {safe_nonnegative_int(event.get('pickup_ebike'))}"
                    )
                    result_text = "已完成"
                history_rows.append(
                    {
    ...
    if isinstance(active_trip, dict):
        ...
        with st.expander("現場變數／修改實際上下車數量", expanded=True):
            ...
            if completed:
                final_truck_bike = truck_bike - actual_unload_bike + actual_pickup_bike
                final_truck_ebike = truck_ebike - actual_unload_ebike + actual_pickup_ebike
                ...
                errors: list[str] = []
                if actual_unload_bike > truck_bike:
                    errors.append("實際卸下的 2.0 超過車上現有數量")
                if actual_unload_ebike > truck_ebike:
                    errors.append("實際卸下的 2.0E 超過車上現有數量")
                ...
                if station_final_bike < 0 or station_final_ebike < 0:
                    errors.append("實際載上數量超過本站作業前可用車數")
...
with st.expander("YouBike 即時車數", expanded=True):
    ...
    auto_refresh_enabled = st.checkbox(
        "每 5 秒自動更新即時車數",
        value=False,
        key=f"auto_youbike_refresh::{current_context_key}",
        help="保留原本手動按鍵；開啟後會由使用者瀏覽器每 5 秒重新同步。官方來源本身若尚未更新，數字可能維持不變。",
    )
    if auto_refresh_enabled:
        st.caption("自動更新已開啟：目的地一旦同意前往會保持鎖定，不會因即時數據變動自行換站。")
    ...
    try:
        ...
        browser_payload = browser_sync_component(
            ...
            max_single_rounds=3,
            wave_delay_ms=70,
            button_label="⚡ 高速取得全部 YouBike 場站車數",
