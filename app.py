...
APP_VERSION = "v28.0-beta1"
APP_VERSION_NAME = "賈維斯語音測試版"
    ...
    st.markdown(
        ...
        <section class="dispatch-hero">
          <div class="dispatch-hero-copy">
            ...
            <h1>臺<span id="jarvis-secret-trigger" title="">東</span> YouBike 智慧調度</h1>
...
(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent=`
      #jarvis-secret-trigger{touch-action:manipulation;user-select:none;-webkit-user-select:none}
      #jarvis-voice-indicator{position:fixed;right:14px;bottom:82px;z-index:2147483000;display:none;
  ...
  function wakeVariants(t){ return /(?:賈維斯|贾维斯|假維斯|假维斯|加維斯|加维斯)/.test(t); }
  function stripWake(t){ return t.replace(/(?:嘿|嗨)?\s*(?:賈維斯|贾维斯|假維斯|假维斯|加維斯|加维斯)[，,。.!！?？\s]*/g,"").trim(); }
  ...
  function startRecognition(){
    ...
    if(!R){ setIndicator("瀏覽器不支援語音",true); return; }
    if(!recognition){
      recognition=new R(); recognition.lang="zh-TW"; recognition.continuous=true; recognition.interimResults=false; recognition.maxAlternatives=1;
      recognition.onstart=()=>{listening=true;setIndicator("賈維斯待命");};
      recognition.onend=()=>{listening=false;if(enabled&&!speaking)win.setTimeout(startRecognition,350);};
      recognition.onerror=(event)=>{listening=false; const e=text(event?.error); if(e==="not-allowed"||e==="service-not-allowed") setIndicator("麥克風權限未開",true); else if(enabled) win.setTimeout(startRecognition,700);};
      recognition.onresult=(event)=>{
        for(let i=event.resultIndex;i<event.results.length;i+=1){
          if(!event.results[i].isFinal) continue;
          const transcript=text(event.results[i][0]?.transcript);
  ...
  function speak(message, after=null){
    ...
    utterance.onend=()=>{speaking=false;setIndicator("賈維斯待命"); if(typeof after==="function") after(); win.setTimeout(startRecognition,250);};
    utterance.onerror=()=>{speaking=false;setIndicator("賈維斯待命");if(typeof after==="function") after();win.setTimeout(startRecognition,350);};
  ...
  function parseActual(raw){
    const normalized=text(raw).replaceAll("班","般").replace(/[，,。]/g," ");
    if(/照原本|照計畫|照计划|一樣|一样/.test(normalized)) return plannedActual();
    if(/都沒有|都没有|零台|沒上下|没上下/.test(normalized)) return emptyActual();
    const result=emptyActual(); let matched=false;
    const parts=normalized.split(/(?=[上下])/).filter(Boolean);
    ...
    for(const part of (parts.length?parts:[normalized])){
      let direction=part.startsWith("上")?"up":part.startsWith("下")?"down":(planOnlyDown?"down":planOnlyUp?"up":"");
      const re=/([零〇一二兩两三四五六七八九十\d]+)\s*(般|電)/g; let m;
      while((m=re.exec(part))!==null){
        const n=parseCnNumber(m[1]); if(n===null||!direction) continue; matched=true;
        const bike=m[2]==="般";
        if(direction==="down") result[bike?"unload_bike":"unload_ebike"]+=n;
  ...
  function handleActualInput(command){
    if(awaitingConfirm){
      if(/^(確認|确认|是|對|对|完成|可以|沒錯|没错)/.test(command)){
        ...
        emit("confirm_completion", {station_name:currentStation(), ...actual});
        return true;
      }
    ...
    if(awaitingActual){
      const actual=parseActual(command);
      if(!actual){ speak("請說實際上幾般幾電，或下幾般幾電"); return true; }
      pendingActual=actual; awaitingActual=false; awaitingConfirm=true;
      speak(`${currentStation()}，${actualPhrase(actual)}，完成，請確認`); return true;
    }
    return false;
  }
  ...
  function handleTranscript(raw){
    ...
    if(awaitingActual || awaitingConfirm){
      const followup=wakeVariants(transcript)?stripWake(transcript):transcript;
      if(handleActualInput(followup)) return;
    }
    if(!wakeVariants(transcript)) return;
    const command=stripWake(transcript);
    if(!command){ speak("我在"); return; }
    if(/請?重複|再說一次|剛剛說什麼|刚刚说什么/.test(command)){ speak(lastSpeech||"目前沒有可重複的內容"); return; }
    if(/有沒有超低電|有没有超低电|超低電有沒有|超低电有没有/.test(command)){ speakUltra(); return; }
    if(/電池|低電|低电/.test(command)){ speakBattery(); return; }
    if(/忘了|忘記|忘记|這裡.*(?:上下|幾台|多少)|这里.*(?:上下|几台|多少)|(?:上下|作業|作业).*(?:多少|幾台|几台)|上多少|下多少|剛剛.*(?:上|下)|刚刚.*(?:上|下)/.test(command)){ speak(stationPhrase(currentPlan||(candidates[0]||null))); return; }
    ...
    if(/我到了|已抵達|已抵达|抵達|抵达|到目的地|到站了|我到站|到了/.test(command)){
      const station=currentStation(); if(!station){speak("目前沒有目標場站");return;}
      if(text(args.mode)==="candidate") speak(`${station}，已抵達`, ()=>emit("lock_station",{station_name:station}));
      else speak(`${station}，已抵達`);
      return;
    }
    if(/有人去|有人處理|有人处理|換下一站|换下一站|改往其他|跳過|跳过|不要去/.test(command)){
      emit("skip_current",{station_name:currentStation(),reason:/有人/.test(command)?"有人已前往":"使用者語音改站"}); return;
    }
    if(/^(?:改往|改去|換去|换去|前往|去)/.test(command)){
      const target=targetTextFromChange(command); emit("change_station",{station_name:currentStation(),target_text:target}); return;
    }
    ...
    speak("我有聽到，但這句目前還沒設定動作");
  }

  function toggleEnabled(){
    enabled=!enabled; try{win.sessionStorage.setItem(STORAGE_KEY,enabled?"1":"0");}catch(_){}
    ensureIndicator();
    if(enabled){ setIndicator("賈維斯待命"); startRecognition(); }
    else { stopRecognition(); win.speechSynthesis.cancel(); setIndicator("賈維斯已關閉"); ensureIndicator().classList.remove("on"); }
  }
  function attachTrigger(){
    const trigger=doc.getElementById("jarvis-secret-trigger"); if(!trigger) return;
    trigger.onpointerdown=()=>{ if(pressTimer) win.clearTimeout(pressTimer); pressTimer=win.setTimeout(()=>{pressTimer=null;toggleEnabled();},Math.max(1200,Number(args.long_press_ms||3000))); };
    const cancel=()=>{ if(pressTimer){win.clearTimeout(pressTimer);pressTimer=null;} };
    trigger.onpointerup=cancel; trigger.onpointerleave=cancel; trigger.onpointercancel=cancel;
  ...
  function hydrate(){
    ...
    ensureIndicator(); attachTrigger(); if(enabled) startRecognition();
    const token=text(args.auto_announce_token);
    if(enabled && args.auto_announce && token && token!==lastAutoAnnounceToken){ lastAutoAnnounceToken=token; win.setTimeout(()=>speakOne(currentPlan||(candidates[0]||null)),450); }
    setHeight();
  }
...
def get_jarvis_voice_component():
    """建立隱藏式賈維斯語音元件；只有長按 Logo 的「東」才會啟用。"""
    global _JARVIS_VOICE_COMPONENT
    if _JARVIS_VOICE_COMPONENT is not None:
        return _JARVIS_VOICE_COMPONENT
    component_dir = Path(tempfile.gettempdir()) / "taitung_jarvis_voice_component_v1"
    component_dir.mkdir(parents=True, exist_ok=True)
    index_path = component_dir / "index.html"
    ...
    _JARVIS_VOICE_COMPONENT = components.declare_component("taitung_jarvis_voice_v1", path=str(component_dir))
    return _JARVIS_VOICE_COMPONENT
    ...
    (() => {
      ...
      page.innerHTML = `
        <header class="battery-header">
          ...
          <div class="battery-title">⚡ 2.0E 電量查詢｜v28.0-beta1</div>
        </header>
...
def jarvis_plan_payload(plan: dict | None) -> dict:
    """只把賈維斯語音需要的最小資料送到瀏覽器，避免傳整張 DataFrame。"""
    if not isinstance(plan, dict):
        return {}
    keys = (
        "station_name", "unload_bike", "unload_ebike", "pickup_bike", "pickup_ebike",
    ...
    *,
    ...
    try:
        ...
        payload = component(
            ...
            default=None,
            mode=mode,
            candidates=[jarvis_plan_payload(plan) for plan in candidates[:SMART_DISPATCH_CANDIDATE_LIMIT]],
            current_plan=jarvis_plan_payload(current_plan),
            threshold=min(100, max(0, int(threshold))),
            priority_threshold=min(int(threshold), max(0, int(priority_threshold))),
            long_press_ms=int(JARVIS_LONG_PRESS_SECONDS * 1000),
            auto_announce=bool(auto_announce),
            auto_announce_token=uuid.uuid4().hex if auto_announce else "",
        )
        ...
        if trip_mode == "一般模式":
            ...
            if isinstance(voice_event, dict):
                ...
                if event_id and st.session_state.get(jarvis_processed_event_key) != event_id:
                    ...
                    if event_type == "confirm_completion":
