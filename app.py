...
def get_dispatch_geolocation_component():
    ...
    component_dir = Path(tempfile.gettempdir()) / "dispatch_geolocation_component_v2"
    component_dir.mkdir(parents=True, exist_ok=True)
    index_path = component_dir / "index.html"
    try:
...
def render_smart_dispatch(
    *,
    ...
    """智慧動態調度：逐站詢問、忽略2回、載量限制、現場變數及完成後重算。"""
    st.markdown("---")
