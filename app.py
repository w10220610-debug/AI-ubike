...
(() => {
  ...
  async function runSync() {
    ...
    try {
      ...
      const batchSize = Math.max(10, Math.min(100, Number(args.batch_size || 50)));
      ...
      const parkingItems = [];
      const groups = batched(catalog.map(item => item.station_id), batchSize);
      for (let index = 0; index < groups.length; index += 1) {
        ...
        if (index < groups.length - 1) await new Promise(resolve => setTimeout(resolve, 180));
      }
      ...
      setStatus(`已取得 ${records.length} 個場站，正在寫入分析系統……`, false);
    } catch (error) {
