...
# 版本：D2／D3 分離配置＋環狀一圈（AI 自動、D2 先行、D3 先行）
...
def render_app_hero() -> None:
    """以純 SVG 建立輕量首頁插畫，不需下載圖片，也不影響操作速度。"""
    st.markdown(
        """
        <section class="dispatch-hero">
          <div class="dispatch-hero-copy">
            ...
            <p>配置、即時車數、辨識與 AI 路線集中在同一套工作流程。</p>
          </div>
...
def adjust_candidates_for_trip_mode(
    ...
    *,
    ...
    for original in candidates:
        ...
        if endpoint_valid:
            straight_distance = haversine_distance_km(
                float(candidate["latitude"]),
                float(candidate["longitude"]),
                float(endpoint_location["latitude"]),
                float(endpoint_location["longitude"]),
            )
            endpoint_distance_km = straight_distance * DISPATCH_ROAD_DISTANCE_FACTOR
            endpoint_drive_minutes = endpoint_distance_km / DISPATCH_ESTIMATED_SPEED_KMH * 60

        # 單趟指定終點只採 35% 方向性權重，避免第一站過度被終點綁死；
        # 來回完整計入返程，環狀則保留 20% 回到維調的方向性，主要順序交由 D2／D3 分階段控制。
        if trip_mode == "來回":
            endpoint_weight = 1.0
        elif trip_mode == "環狀一圈":
        ...
        route_total_minutes = float(candidate["estimated_total_minutes"]) + endpoint_drive_minutes * endpoint_weight
...
def loop_movement_direction(start_name: str, phase_index: int) -> int:
    """台東出發第一段往北、池上出發第一段往南；跨區後反向回到出發維調。"""
    first_direction = 1 if str(start_name).strip() == "台東維調" else -1
    return first_direction if phase_index <= 0 else -first_direction
...
def adjust_candidates_for_loop_direction(
    ...
    *,
    ...
    """環狀模式限制同一側沿既定方向前進，避免即時數據更新後反覆折返。"""
    if movement_direction not in (-1, 1) or not location_payload_is_valid(current_location):
        return candidates
    ...
    for original in candidates:
        candidate = dict(original)
        progress_km = (float(candidate.get("latitude") or 0) - current_latitude) * 111.0 * movement_direction
        if progress_km >= 0:
            direction_multiplier = min(1.18, 1.04 + progress_km / 180.0)
        elif progress_km >= -1.5:
            direction_multiplier = 0.72
        else:
            direction_multiplier = 0.24
        candidate["loop_progress_km"] = progress_km
        candidate["loop_direction_multiplier"] = direction_multiplier
        candidate["score"] = float(candidate.get("score") or 0) * direction_multiplier
        adjusted.append(candidate)
    ...
    return sorted(
        ...
        key=lambda item: (
            float(item.get("score") or 0),
            safe_nonnegative_int(item.get("dispatch_count")),
            float(item.get("loop_progress_km") or 0),
            -float(item.get("estimated_distance_km") or 0),
        ),
        reverse=True,
    )
    ...
    *,
    ...
    for zone in zone_sequence:
        while len(preview) < max(1, max_stops):
            ...
            if zone:
                candidates = adjust_candidates_for_loop_direction(
                    candidates,
                    current_location=simulated_location,
                    movement_direction=loop_movement_direction(loop_start_name, phase_index),
                )
            if not candidates:
                break
...
def render_long_distance_route_preview(
    ...
    *,
        ...
        preview_rows.append(
            {
                ...
                "距離(km)": round(float(plan.get("estimated_distance_km") or 0), 1),
    ...
    st.caption("此為目前即時資料的路線預覽；每完成一站或資料明顯變動後，只會重排目前階段與後續路線。")
    ...
    *,
    ...
    for _row_index, row in valid_df.iterrows():
        ...
        if (
            ...
            and safe_nonnegative_int(cooldown.get("resume_after_round")) >= current_round
        ):
            continue
...
def render_smart_dispatch(
    *,
    ...
    st.caption(
        page_caption
        or "每次只安排一站；同意後鎖定目的地，完成本站才重新計算。距離與時間為官方座標搭配道路係數的估算值，導航請以地圖實際路況為準。"
    )
    ...
    if not candidates:
        if trip_mode == "環狀一圈" and resolved_loop_order and active_loop_phase == resolved_loop_order[-1]:
            ...
            st.warning(
                "目前找不到可執行的下一站。可能原因：全部符合配置、車上車種不足、貨車已滿、場站仍在忽略回合，或部分場站缺少座標／現況資料。"
            )
        render_dispatch_auxiliary_panels(
            dispatch_prefix=dispatch_prefix,
            cooldown_key=cooldown_key,
    ...
    if allow_manual_station_choice:
        with st.expander("改選其他場站", expanded=False):
            candidate_names = [candidate["station_name"] for candidate in candidates]
            default_index = candidate_names.index(manual_station_name) if manual_station_name in candidate_names else 0
        ...
        with st.expander("查看其他候選場站", expanded=False):
            ...
            for rank, candidate in enumerate(candidates[:8], start=1):
                candidate_rows.append(
                    {
                        "順位": rank,
                        "區域": candidate.get("route_zone", ""),
                        "場站": candidate["station_name"],
                        "調度量": candidate["dispatch_count"],
                        "距離(km)": round(candidate["estimated_distance_km"], 1),
                        "總時間(分)": round(candidate.get("route_total_minutes", candidate["estimated_total_minutes"])),
...
def render_long_distance_route_page(
    *,
    ...
    st.markdown(
        ...
        background:linear-gradient(135deg,rgba(22,119,255,.08),rgba(16,185,129,.05));margin:.35rem 0 .9rem;">
          ...
          <div style="margin-top:.3rem;font-size:.86rem;opacity:.72;line-height:1.55;">
            此頁只處理 D2、D3 長距離調度；一般分析、照片辨識與手動輸入仍留在「一般分析」頁。
          </div>
        </section>
        """,
        unsafe_allow_html=True,
    ...
    render_smart_dispatch(
        ...
        page_caption=(
            "AI 會先產生完整預覽，但每完成一站、每次即時數據更新或你手動改選場站後，"
            "都會重新安排後續路線；環狀模式只重排目前區域與後續區域，不會跨山後再折返。"
        ),
