...
def _apply_status_lookup(target_df: pd.DataFrame, source_df: pd.DataFrame) -> pd.DataFrame:
    """以行政區＋場站名稱向量化套用現況，避免逐列 iterrows。"""
    result_df = target_df.copy()
    required_columns = ["行政區", "場站名稱", *CURRENT_STATUS_COLUMNS]
    if source_df.empty or any(column not in source_df.columns for column in required_columns):
        return coerce_nullable_current_status(result_df)

    lookup_df = source_df[required_columns].copy()
    lookup_df["行政區"] = lookup_df["行政區"].astype(str)
    lookup_df["場站名稱"] = lookup_df["場站名稱"].astype(str)
    lookup_df = coerce_nullable_current_status(lookup_df)
    lookup_df = lookup_df.drop_duplicates(["行政區", "場站名稱"], keep="last")
    lookup_df = lookup_df.set_index(["行政區", "場站名稱"])

    target_keys = pd.MultiIndex.from_frame(
        result_df[["行政區", "場站名稱"]].astype(str),
        names=["行政區", "場站名稱"],
    )
...
def restore_current_status(base_df: pd.DataFrame, saved_records) -> pd.DataFrame:
    ...
    if not valid_records:
        return coerce_nullable_current_status(base_df.copy())
