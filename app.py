...
(() => {
  ...
  let candidates = [];
  const CONTEXT_KEY = "__taitungJarvisDispatchContextV28";
  ...
  function standbyLabel(){ const station=currentStation(); return station ? `賈維斯待命｜${station}` : "賈維斯待命｜調度資料更新中"; }
  ...
  function startRecognition(){
    ...
    if(!recognition){
      recognition=new R(); recognition.lang="zh-TW"; recognition.continuous=!isIOS(); recognition.interimResults=false; recognition.maxAlternatives=1;
  ...
  function handleTranscript(raw){
    ...
    // V28.4：直接說「上1般2電／下2般1電」即可把數量填進目前場站表單，但不直接完成本站。
    if(/[上下]/.test(command) && /(般|班|電)/.test(command)){
      const actual=parseActual(command);
      if(actual){
        const station=currentStation();
        if(!station){ speak("調度資料更新中，請稍後再說一次"); return; }
        if(text(args.mode)!=="active"){ speak(`${station}，請先鎖定目標場站再輸入實際上下車數量`); return; }
        speak(`${station}，${actualPhrase(actual)}，已填入，請確認`, ()=>emit("fill_actual", {station_name:station, ...actual}));
        return;
      }
    }
    if(/[上下].*(?:\d+|[一二兩两三四五六七八九十]).*台/.test(command) && !/(般|班|電)/.test(command)){
      speak("請說般或電，例如上兩般一電");
      return;
    }
    if(/完成|做完|處理完|处理完/.test(command)){
      if(!currentStation()){speak("調度資料更新中，請稍後再說一次");return;}
    ...
    if(/我到了|已抵達|已抵达|抵達|抵达|到目的地|到站了|我到站|到了/.test(command)){
      const station=currentStation(); if(!station){speak("調度資料更新中，請稍後再說一次");return;}
  ...
  function hydrate(){
    const incomingPlan=args.current_plan&&typeof args.current_plan==="object"?args.current_plan:null;
    const incomingCandidates=Array.isArray(args.candidates)?args.candidates:[];
    const hasIncoming=Boolean(text((incomingPlan||{}).station_name)) || incomingCandidates.some(x=>Boolean(text((x||{}).station_name)));
    if(hasIncoming){
      currentPlan=incomingPlan;
      candidates=incomingCandidates;
      try{ win[CONTEXT_KEY]={currentPlan,candidates,mode:text(args.mode),updatedAt:Date.now()}; }catch(_){}
    }else{
      const saved=win[CONTEXT_KEY];
      if(saved && typeof saved==="object" && Date.now()-Number(saved.updatedAt||0)<120000){
        currentPlan=saved.currentPlan&&typeof saved.currentPlan==="object"?saved.currentPlan:null;
...
def get_jarvis_voice_component():
    ...
    component_dir = Path(tempfile.gettempdir()) / "taitung_jarvis_voice_component_v2"
    component_dir.mkdir(parents=True, exist_ok=True)
    index_path = component_dir / "index.html"
    ...
    _JARVIS_VOICE_COMPONENT = components.declare_component("taitung_jarvis_voice_v2", path=str(component_dir))
    return _JARVIS_VOICE_COMPONENT
...
def jarvis_plan_payload(plan: dict | None) -> dict:
    ...
    V28.4 延續既有執行中資料，不要求配置表新增欄位。這份 payload 之後可直接
    ...
    *,
    ...
    try:
        ...
        payload = component(
            ...
            candidate_count=min(len(candidates), SMART_DISPATCH_CANDIDATE_LIMIT),
