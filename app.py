...
def render_binding_vehicle_requirements(status_df: pd.DataFrame, *, selected_shift: str) -> None:
    """只顯示哪些場站要綁多少台，不顯示任何總數。"""
    if status_df.empty or "綁車需求" not in status_df.columns or "場站名稱" not in status_df.columns:
        return
    items: list[tuple[str, str, str]] = []
    for route_zone, station_name, raw_requirement in status_df.reindex(
        columns=["路線區域", "場站名稱", "綁車需求"]
    ).itertuples(index=False, name=None):
        display = binding_requirement_display_text(raw_requirement)
        station_name = str(station_name or "").strip()
        if station_name and display:
            items.append((str(route_zone or "").strip(), station_name, display))
    if not items:
        return
    cards: list[str] = []
    for route_zone, station_name, display in items:
        zone_html = (
            f'<span style="opacity:.62;font-size:.72rem;">{html.escape(route_zone)}</span>'
            if route_zone else ""
        )
        cards.append(
            '<div style="border:1px solid rgba(255,84,183,.32);background:rgba(255,84,183,.07);'
            'border-radius:12px;padding:.65rem .72rem;min-width:0;">'
            f'<div style="font-weight:900;line-height:1.32;">{html.escape(station_name)}</div>'
            '<div style="display:flex;justify-content:space-between;gap:.55rem;align-items:center;margin-top:.32rem;">'
            f'{zone_html}<strong style="color:#ff8ed8;white-space:nowrap;">🔒 {html.escape(display)}</strong>'
            '</div></div>'
        )
    panel_html = (
        '<section style="border:1px solid rgba(255,84,183,.34);border-radius:16px;padding:.78rem .82rem;'
        'background:linear-gradient(135deg,rgba(255,84,183,.08),rgba(78,231,255,.035));margin:.55rem 0 .8rem;">'
        f'<div style="font-weight:950;font-size:1.02rem;margin-bottom:.5rem;">🔒 綁車需求｜{html.escape(str(selected_shift))}</div>'
...
def _build_analysis_result_table_html(rows: tuple[tuple, ...]) -> str:
    ...
    for (
        ...
        total_bikes_raw, empty_spaces_raw, binding_requirement_raw, distance_raw, drive_minutes_raw,
    ) in rows:
        ...
        binding_text = binding_requirement_display_text(binding_requirement_raw)
        if binding_text:
            station_meta.append(f"🔒 綁車 {html.escape(binding_text)}")
        if str(distance_raw) != "—":
...
def render_analysis_result_table(region_df: pd.DataFrame) -> None:
    ...
    for column in (
        "路線區域", "2.0 現況", "2.0 標準", "2.0E 現況", "2.0E 標準",
        "總車數", "空位數", "綁車需求", "距離目前位置 (km)", "預估行車時間 (分)",
    ):
    ...
    for position, values in enumerate(
        ...
        (
            ...
            total_bikes, empty_spaces, binding_requirement,
        ) = values
        ...
        rows.append((
            ...
            normalize_binding_requirement(binding_requirement),
...
def merge_battery_route_station_maps(
    ...
    for zone in ALL_DISPATCH_ZONES:
        for item in (primary_map.get(zone, []) if isinstance(primary_map, dict) else []):
...
def visible_configuration_sheet_names(options: list[tuple[str, str]]) -> list[str]:
    """配置類型直接跟 Excel 顯示中的工作表同步；隱藏工作表不會出現在這裡。"""
    return list(dict.fromkeys(str(sheet_name) for sheet_name, _route in options if str(sheet_name).strip()))


def configuration_schedule_tags(sheet_name: str) -> set[str]:
    """從工作表名稱取出班表性質；「平假日」視為可同時搭配平日與假日。"""
    normalized = re.sub(r"\s+", "", str(sheet_name or "")).lower()
    tags: set[str] = set()
    if "平假日" in normalized or "平假" in normalized:
        tags.update(("平日", "假日"))
    else:
        if "平日" in normalized or "weekday" in normalized or "週間" in normalized:
            tags.add("平日")
        if any(token in normalized for token in ("假日", "週末", "周末", "holiday", "weekend")):
            tags.add("假日")
    if "暑假" in normalized or "夏季" in normalized or "summer" in normalized:
        tags.add("暑假")
    if "寒假" in normalized or "winter" in normalized:
        tags.add("寒假")
    return tags


def configuration_sheet_coverage(options: list[tuple[str, str]]) -> dict[str, set[str]]:
    coverage: dict[str, set[str]] = {}
    for sheet_name, route in options:
        zone = normalize_dispatch_zone(route)
        if zone:
            coverage.setdefault(str(sheet_name), set()).add(zone)
    return coverage


def ranked_companion_sheets(
    options: list[tuple[str, str]],
    selected_sheet: str,
    zone: str,
) -> list[tuple[str, int]]:
    """為選定工作表缺少的區域找最相容的可見工作表；不讀取隱藏頁。"""
    selected_tags = configuration_schedule_tags(selected_sheet)
    order: list[str] = []
    for sheet_name, route in options:
        sheet_name = str(sheet_name)
        if normalize_dispatch_zone(route) != zone or sheet_name == selected_sheet:
            continue
        if sheet_name not in order:
            order.append(sheet_name)
    scored: list[tuple[str, int, int]] = []
    for sheet_name in order:
        candidate_tags = configuration_schedule_tags(sheet_name)
        score = 0
        if selected_tags and candidate_tags:
            overlap = selected_tags & candidate_tags
            score += 100 + 20 * len(overlap) if overlap else -100
            if {"平日", "假日"}.issubset(candidate_tags):
                score += 25
        elif not selected_tags and not candidate_tags:
            score += 10
        elif not candidate_tags:
            score += 5
        scored.append((sheet_name, score, order.index(sheet_name)))
    scored.sort(key=lambda item: (-item[1], item[2]))
    return [(sheet_name, score) for sheet_name, score, _ in scored]


def build_configuration_options_for_sheet(
    options: list[tuple[str, str]],
    selected_sheet: str,
    companion_overrides: dict[str, str] | None = None,
) -> list[tuple[str, str]]:
    """把選中的可見頁與必要搭配頁組成完整 D1＋D2＋D3，不犧牲原本全區分析。"""
    companion_overrides = companion_overrides if isinstance(companion_overrides, dict) else {}
    output: list[tuple[str, str]] = []
    for zone in ALL_DISPATCH_ZONES:
        same = next(((s, r) for s, r in options if str(s) == str(selected_sheet) and normalize_dispatch_zone(r) == zone), None)
        if same is not None:
            output.append(same)
            continue
        override = str(companion_overrides.get(zone) or "").strip()
        if override:
            match = next(((s, r) for s, r in options if str(s) == override and normalize_dispatch_zone(r) == zone), None)
            if match is not None:
                output.append(match)
                continue
        ranked = ranked_companion_sheets(options, selected_sheet, zone)
        if not ranked:
            continue
        companion = ranked[0][0]
        match = next(((s, r) for s, r in options if str(s) == companion and normalize_dispatch_zone(r) == zone), None)
        if match is not None:
            output.append(match)
    return output
...
def build_dispatch_plan_for_station(
    ...
    *,
    ...
    return {
        ...
        "binding_requirement": normalize_binding_requirement(row.get("綁車需求")),
...
def render_station_inspection_card(inspection: dict) -> None:
    ...
    binding_text = binding_requirement_display_text(inspection.get("binding_requirement"))
    binding_line = f"<br><strong>🔒 綁車需求：{html.escape(binding_text)}</strong>" if binding_text else ""
    st.markdown(
        f"""
        <section class="dispatch-plan-card" style="margin:.45rem 0 .7rem;">
          ...
          <div class="dispatch-plan-action">
            ...
            總車 {optional_count_text(inspection.get('total_bikes'))} 台｜空位 {optional_count_text(inspection.get('empty_spaces'))} 格｜總柱 {optional_count_text(inspection.get('parking_spaces'))} 格{binding_line}<br>
...
def render_dispatch_plan_card(plan: dict, *, title: str) -> None:
    ...
    binding_text = binding_requirement_display_text(plan.get("binding_requirement"))
    binding_line = f"<br><strong>🔒 綁車需求：{html.escape(binding_text)}</strong>" if binding_text else ""
    ...
    st.markdown(
        ...
        <section class="dispatch-plan-card" data-ubike-station-name="{station_name_attr}">
          ...
          <div class="dispatch-plan-action">
            ...
            總車 {optional_count_text(plan.get('total_bikes'))} 台｜空位 {optional_count_text(plan.get('empty_spaces'))} 格｜總柱 {optional_count_text(plan.get('parking_spaces'))} 格{binding_line}
          </div>
    ...
    *,
    ...
    if visible_candidates:
        ...
        for rank, candidate in enumerate(visible_candidates, start=1):
            ...
            binding_text = binding_requirement_display_text(candidate.get("binding_requirement"))
            binding_line = f"  \n🔒 綁車需求：{binding_text}" if binding_text else ""
            card_label = (
                ...
                f"總柱 {optional_count_text(candidate.get('parking_spaces'))} 格{binding_line}  \n"
...
def normalize_binding_requirement(value) -> str:
    """保留配置表原始綁車寫法（例如 20、5+5、電車5）；空白或 0 不顯示。"""
    if value is None:
        return ""
    try:
        if pd.isna(value):
    ...
    try:
        ...
        if numeric <= 0:
            return ""
        return str(int(numeric)) if numeric.is_integer() else compact
    except (TypeError, ValueError, OverflowError):
        return value_text


def binding_requirement_display_text(value) -> str:
    requirement = normalize_binding_requirement(value)
    if not requirement:
        return ""
    if re.fullmatch(r"[0-9.+＋]+", requirement):
        return f"{requirement.replace('＋', '+')} 台"
    return requirement


def binding_requirement_map_from_sheet(sheet_df: pd.DataFrame, route: str, shift: str) -> dict[str, str]:
    """直接從目前可見工作表抓指定班別的 2.0綁車 欄。"""
    if sheet_df.empty or sheet_df.shape[1] < 3:
        return {}
    zone = normalize_dispatch_zone(route)
    if not zone:
        return {}
    shift_text = re.sub(r"\s+", "", str(shift or ""))
    binding_column: int | None = None
    for row_index in range(len(sheet_df)):
        row_values = sheet_df.iloc[row_index].tolist()
        for column_index, raw in enumerate(row_values):
            header = re.sub(r"\s+", "", str(raw or ""))
            if not header or not shift_text or shift_text not in header or "配置" not in header:
                continue
            for candidate_index in range(column_index + 1, min(column_index + 5, len(row_values))):
                candidate_header = re.sub(r"\s+", "", str(row_values[candidate_index] or ""))
                if "綁車" in candidate_header:
                    binding_column = candidate_index
                    break
            if binding_column is not None:
                break
        if binding_column is not None:
            break
    if binding_column is None:
        return {}

    result: dict[str, str] = {}
    active_zone: str | None = None
    for row_index in range(len(sheet_df)):
        first_value = sheet_df.iat[row_index, 0]
        marker = normalize_dispatch_zone(first_value)
        marker_text = re.sub(r"\s+", "", str(first_value or "").upper())
        if marker and marker_text == marker:
            active_zone = marker
            continue
        if active_zone != zone:
            continue
        station_name = str(sheet_df.iat[row_index, 2] or "").strip()
        if not station_name or station_name == "場站名稱":
            continue
        requirement = normalize_binding_requirement(sheet_df.iat[row_index, binding_column])
        if requirement:
            result[normalize_youbike_station_key(station_name)] = requirement
    return result


def attach_binding_requirements(route_df: pd.DataFrame, sheet_df: pd.DataFrame, route: str, shift: str) -> pd.DataFrame:
    output = route_df.copy()
    requirement_map = binding_requirement_map_from_sheet(sheet_df, route, shift)
    if "場站名稱" not in output.columns:
        output["綁車需求"] = ""
        return output
    output["綁車需求"] = [
        requirement_map.get(normalize_youbike_station_key(name), "")
        for name in output["場站名稱"].astype(str)
    ]
    return output
...
def cached_parse_route(source: bytes | str, sheet_name: str, route: str, shift: str) -> pd.DataFrame:
    """快取已解析的分區資料，並附上該班別綁車需求。"""
    workbook = cached_load_workbook(source)
    raw_sheet = workbook[sheet_name]
    route_df = parse_route(raw_sheet.copy(deep=True), route, shift)
    return attach_binding_requirements(route_df, raw_sheet, route, shift)
...
if active_base is None:
    render_floating_battery_query(DEFAULT_BATTERY_ROUTE_STATION_MAP, mobile_mode)
    if base_token:
        st.warning("瀏覽器已找回先前的配置識別碼，但伺服器暫存檔已不存在；請重新上傳配置表。⚡ 電量查詢仍可直接使用。")
    else:
        st.info("請上傳配置表以啟用一般分析／智慧調度。右側 ⚡ 電量查詢目前已可直接使用。")
    st.stop()
...
if st.session_state.get(configuration_state_key) not in visible_sheet_choices:
    st.session_state[configuration_state_key] = visible_sheet_choices[0]

with st.container(border=True):
    selected_configuration_type = st.selectbox(
        "配置類型",
        visible_sheet_choices,
        key=configuration_state_key,
        help="此清單完全跟 Excel 目前『顯示中』的工作表同步；隱藏頁不讀取也不顯示。",
    )
    selected_sheet = selected_configuration_type
    coverage = configuration_sheet_coverage(options)
    companion_overrides: dict[str, str] = {}
    missing_for_selected = [zone for zone in ALL_DISPATCH_ZONES if zone not in coverage.get(selected_sheet, set())]
    for missing_zone in missing_for_selected:
        ranked = ranked_companion_sheets(options, selected_sheet, missing_zone)
        if not ranked:
            continue
        top_score = ranked[0][1]
        tied_top = [sheet_name for sheet_name, score in ranked if score == top_score]
        if len(tied_top) > 1:
            companion_key = f"configuration_companion::{active_base['token']}::{selected_sheet}::{missing_zone}"
            if st.session_state.get(companion_key) not in tied_top:
                st.session_state[companion_key] = tied_top[0]
            companion_overrides[missing_zone] = st.selectbox(
                f"{missing_zone} 搭配配置",
                tied_top,
                key=companion_key,
                help=f"目前選的頁面沒有 {missing_zone}，且有多張同樣適合的可見頁，因此由你指定搭配。",
            )
    ...
    page_mode = st.radio(
        ...
        ["一般分析", "智慧調度"],
        horizontal=True,
        key=(f"page_mode::{active_base['token']}::{selected_configuration_type}::{selected_shift}"),
    )

configuration_options = build_configuration_options_for_sheet(
    options, selected_sheet, companion_overrides=companion_overrides
)
available_zone_set = {
    zone
