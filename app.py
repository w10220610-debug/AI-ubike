...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    #syncButton {
      width: 100%; min-height: 58px; border: 0; border-radius: 13px;
      padding: 12px 16px; font-size: 18px; font-weight: 700; line-height: 1.25;
      color: #fff; background: #ff4b4b; cursor: pointer;
      -webkit-tap-highlight-color: transparent; touch-action: manipulation;
    }
    #syncButton:disabled { opacity: .72; cursor: wait; }
    #status { min-height: 18px; margin-top: 6px; padding: 0 3px; font-size: 13px; color: #6b7280; }
    .error { color: #c62828 !important; }
  </style>
</head>
<body>
  <button id="syncButton" type="button">🔄 更新即時車數</button>
  <div id="status"></div>
<script>
(() => {
  const API_VERSION = 1;
  ...
  function setHeight() {
    send("streamlit:setFrameHeight", { height: Math.max(82, document.body.scrollHeight + 2) });
  }
  ...
  async function runSync() {
    ...
    button.textContent = "⏳ 正在由瀏覽器讀取 YouBike 官網……";
    setStatus("正在取得 YouBike 官網資料……", false);
    ...
    try {
      ...
      // 官網接口偶爾會對大型批次只回傳部分站點；採較小批次降低漏站機率。
      const batchSize = Math.max(1, Math.min(20, Number(args.batch_size || 10)));
      ...
      setStatus(`已找到 ${catalog.length} 個臺東候選場站，正在取得 2.0／2.0E 車數……`, false);

      const parkingItems = [];
      const requestedStationIds = catalog.map(item => item.station_id);
      const groups = batched(requestedStationIds, batchSize);
      for (let index = 0; index < groups.length; index += 1) {
        const parkingPayload = await fetchJson(parkingUrl, {
          method: "POST",
          headers: {
            "Accept": "application/json, text/plain, */*",
            "Content-Type": "application/json;charset=UTF-8",
          },
          body: JSON.stringify({ station_no: groups[index] }),
        });
        parkingItems.push(...extractItems(parkingPayload));
        if (index < groups.length - 1) await new Promise(resolve => setTimeout(resolve, 220));
      }

      // 不直接相信大型批次一定完整：核對回傳站號，缺少的站點再以小批次補查一次。
      const receivedStationIds = new Set(
        parkingItems
          .map(item => String(firstNonempty(item.station_no, item.sno) || "").trim())
          .filter(Boolean)
      );
      let missingStationIds = requestedStationIds.filter(stationId => !receivedStationIds.has(stationId));
      if (missingStationIds.length) {
        setStatus(`第一輪尚缺 ${missingStationIds.length} 個場站，正在自動補查……`, false);
        const retryGroups = batched(missingStationIds, 5);
        for (let index = 0; index < retryGroups.length; index += 1) {
          const retryPayload = await fetchJson(parkingUrl, {
            method: "POST",
            headers: {
              "Accept": "application/json, text/plain, */*",
              "Content-Type": "application/json;charset=UTF-8",
            },
            body: JSON.stringify({ station_no: retryGroups[index] }),
          });
          parkingItems.push(...extractItems(retryPayload));
          if (index < retryGroups.length - 1) await new Promise(resolve => setTimeout(resolve, 320));
        }
      }

      const parkingMap = new Map();
      for (const item of parkingItems) {
        const stationId = String(firstNonempty(item.station_no, item.sno) || "").trim();
        if (stationId) parkingMap.set(stationId, item);
      }
      ...
      missingStationIds = requestedStationIds.filter(stationId => !parkingMap.has(stationId));
      setValue({
        ok: true,
        event_id: eventId(),
        records,
        ...
        request_batch_count: groups.length,
        source: "YouBike 官網公開接口（由使用者瀏覽器直接取得，免 TDX）",
      });
      const missingText = missingStationIds.length ? `，仍缺 ${missingStationIds.length} 個` : "";
      setStatus(`已取得 ${records.length} 個場站${missingText}，正在寫入分析系統……`, false);
    } catch (error) {
      ...
      button.textContent = args.button_label || "🔄 更新即時車數";
      setHeight();
    }
  }
...
def get_youbike_browser_sync_component():
    ...
    component_dir = Path(tempfile.gettempdir()) / "youbike_browser_sync_component_v1"
...
def normalize_browser_live_payload(payload) -> dict:
    ...
    return {
        ...
        "source": str(payload.get("source") or "YouBike 官網公開接口（由瀏覽器直接取得，免 TDX）"),
...
def build_emergency_station_snapshot(
    route_station_map: dict[str, list[dict]],
    live_records: list[dict],
    previous_stations,
) -> list[dict]:
    """整理目前無車／無空位場站，並讓同一事件保留原始開始時間。"""
    previous_by_station = {
        str(record.get("station_id") or record.get("station_name") or ""): record
        for record in (previous_stations or [])
        if isinstance(record, dict)
    }
    now_timestamp = time.time()
    emergency_stations: list[dict] = []
    seen_station_ids: set[str] = set()

    for route in ("D1", "D2", "D3"):
        for station in route_station_map.get(route, []):
            station_name = str(station.get("name") or "").strip()
            if not station_name:
                continue

            matched_record, _, ambiguous = match_youbike_station(station_name, live_records)
            if matched_record is None or ambiguous:
                continue
            if safe_nonnegative_int(matched_record.get("service_status")) != 1:
                continue

            station_id = str(matched_record.get("station_id") or station_name).strip()
            if station_id in seen_station_ids:
                continue

            available_bikes = normalize_current_status(matched_record.get("available_spaces"))
            if available_bikes is None:
                general_bikes = normalize_current_status(matched_record.get("general_bikes"))
                electric_bikes = normalize_current_status(matched_record.get("electric_bikes"))
                if general_bikes is not None and electric_bikes is not None:
                    available_bikes = general_bikes + electric_bikes
            empty_spaces = normalize_current_status(matched_record.get("empty_spaces"))

            conditions: list[str] = []
            if available_bikes == 0:
                conditions.append("沒有車")
            if empty_spaces == 0:
                conditions.append("沒有空位")
            if not conditions:
                continue

            previous = previous_by_station.get(station_id, {})
            detected_at = previous.get("detected_at", now_timestamp)
            try:
            ...
            emergency_stations.append(
                {
                    ...
                    "station_name": station_name,
                    "route": route,
                    "district": str(station.get("district") or "").strip(),
...
def render_emergency_station_timers(emergency_stations: list[dict]) -> None:
    """為每個無車／無空位場站建立可暫停、繼續與重新計時的獨立計時器。"""
    payload = json.dumps(emergency_stations, ensure_ascii=False).replace("</", "<\\/")
    component_html = r"""
    <!doctype html>
    <html lang="zh-Hant">
    <head>
      ...
      <style>
        * { box-sizing: border-box; }
        ...
        .timer-card {
          ...
          border-radius: 13px;
          background: #fff;
          box-shadow: 0 4px 14px rgba(35, 40, 48, .08);
        }
        .timer-card.no-space { border-left-color: #7c3aed; }
        .timer-top { display: flex; justify-content: space-between; gap: 8px; align-items: flex-start; }
        .timer-route { color: #6b7280; font-size: 12px; font-weight: 800; }
        .timer-name { margin-top: 2px; font-size: 16px; font-weight: 850; line-height: 1.3; }
        .timer-condition {
          flex: 0 0 auto;
          padding: 4px 8px;
          border-radius: 999px;
        ...
        .timer-card.no-space .timer-condition { color: #5b21b6; background: #ede9fe; }
        .timer-value {
          margin: 10px 0 9px;
          color: #111827;
        ...
        .timer-actions button.primary { border-color: #ffb4b4; background: #fff0f0; }
    ...
    <body>
      ...
      <script>
      (() => {
        const stations = __EMERGENCY_DATA__;
        const grid = document.getElementById("timerGrid");
        const storagePrefix = "ubike-emergency-timer-v1::";

        function readState(key, fallbackStart) {
          try {
            const saved = JSON.parse(localStorage.getItem(key) || "null");
            if (saved && Number.isFinite(saved.startedAt)) return saved;
          } catch (_) {}
          return { startedAt: fallbackStart, pausedAt: null, pausedTotal: 0 };
        }
        function writeState(key, state) {
          try { localStorage.setItem(key, JSON.stringify(state)); } catch (_) {}
        }
        function elapsedMs(state) {
          const end = Number.isFinite(state.pausedAt) ? state.pausedAt : Date.now();
          return Math.max(0, end - state.startedAt - Number(state.pausedTotal || 0));
        }
        ...
        for (const station of stations) {
          const incidentStart = Math.max(0, Number(station.detected_at || 0) * 1000) || Date.now();
          const incidentId = `${station.station_id || station.station_name}::${Math.floor(incidentStart)}`;
          const storageKey = storagePrefix + incidentId;
          let state = readState(storageKey, incidentStart);

          const card = document.createElement("section");
          if ((station.conditions || []).includes("沒有空位")) card.classList.add("timer-card", "no-space");
          else card.classList.add("timer-card");

          const top = document.createElement("div");
          top.className = "timer-top";
          const nameWrap = document.createElement("div");
          const route = document.createElement("div");
          route.className = "timer-route";
          route.textContent = [station.route, station.district].filter(Boolean).join("｜");
          const name = document.createElement("div");
          name.className = "timer-name";
          name.textContent = station.station_name || "未命名場站";
          nameWrap.append(route, name);
          const condition = document.createElement("span");
          condition.className = "timer-condition";
          condition.textContent = (station.conditions || []).join("／");
          top.append(nameWrap, condition);
          ...
          pauseButton.className = "primary";
          const resetButton = document.createElement("button");
          resetButton.textContent = "重新計時";
          actions.append(pauseButton, resetButton);
          card.append(top, value, actions);
          grid.appendChild(card);

          function render() {
            value.textContent = formatDuration(elapsedMs(state));
            pauseButton.textContent = Number.isFinite(state.pausedAt) ? "繼續" : "暫停";
          }
          pauseButton.addEventListener("click", () => {
            if (Number.isFinite(state.pausedAt)) {
              state.pausedTotal = Number(state.pausedTotal || 0) + Date.now() - state.pausedAt;
              state.pausedAt = null;
            } else {
          ...
          resetButton.addEventListener("click", () => {
            state = { startedAt: Date.now(), pausedAt: null, pausedTotal: 0 };
            writeState(storageKey, state);
            render();
          });
          render();
    ...
    """.replace("__EMERGENCY_DATA__", payload)

    component_height = min(720, max(148, 12 + len(emergency_stations) * 148))
...
def render_floating_battery_query(
    ...
    (() => {
      ...
      const preferenceKey = "ubike-battery-query-preferences-v1";
      ...
      style.textContent = `
        ...
        #ubike-battery-page .battery-control-grid {
          display: grid;
          grid-template-columns: minmax(180px, .8fr) minmax(260px, 1.5fr) minmax(160px, .7fr);
          gap: 13px;
          align-items: end;
        }
      ...
      page.innerHTML = `
        <header class="battery-header">
          ...
          <div class="battery-title">⚡ 2.0E 電量查詢</div>
        </header>
        <main class="battery-main">
          <section class="battery-control-card">
            <div class="battery-control-grid">
              <label>
                ...
                <span class="battery-threshold-wrap">
                  <input class="battery-threshold" type="number" min="0" max="100" step="1" inputmode="numeric" value="30" />
                  <strong>%</strong>
                </span>
              </label>
              <div>
            ...
            <div class="battery-inclusive">門檻包含設定值，例如 30% 會列出電量 ≤ 30% 的車輛。</div>
          </section>
          <div class="battery-status">準備查詢……</div>
      ...
      function clampThreshold(value) {
        ...
        return Math.max(0, Math.min(100, Number.isFinite(number) ? number : 30));
      }
      ...
      function renderResults() {
        ...
        for (const result of lastResults) {
          ...
          totals[result.route] += result.bikes.filter(bike => bike.battery_power <= threshold).length;
        }
        ...
        for (const route of lastResultRoutes) {
          appendTextElement(resultsNode, "div", "battery-route-title", `${route}｜${totals[route] || 0} 顆需更換`);
          const routeResults = lastResults
            .filter(result => result.route === route)
            .sort((left, right) => {
              const leftCount = left.bikes.filter(bike => bike.battery_power <= threshold).length;
              const rightCount = right.bikes.filter(bike => bike.battery_power <= threshold).length;
              return rightCount - leftCount || left.name.localeCompare(right.name, "zh-Hant");
            });

          for (const result of routeResults) {
            const details = doc.createElement("details");
            details.className = "battery-station" + (result.matched ? "" : " unmatched");
            const summary = doc.createElement("summary");
            appendTextElement(summary, "span", "battery-station-name", result.name);
            const lowBikes = result.bikes
              .filter(bike => bike.battery_power <= threshold)
              .sort((left, right) => left.battery_power - right.battery_power || left.bike_no.localeCompare(right.bike_no));
            const count = appendTextElement(
              summary,
              "span",
              "battery-count",
              result.matched ? `需換 ${lowBikes.length} 顆` : "查無場站",
            );
            if (!result.matched) count.classList.add("unknown");
            else if (lowBikes.length) count.classList.add("needs-change");
            details.appendChild(summary);

            const list = doc.createElement("div");
            list.className = "battery-bike-list";
            if (!result.matched) {
              appendTextElement(list, "div", "battery-empty", "配置表名稱無法安全配對官網站名，本次未查詢。");
            } else if (!lowBikes.length) {
              appendTextElement(list, "div", "battery-empty", `沒有電量 ≤ ${threshold}% 的車輛。`);
            } else {
              for (const bike of lowBikes) {
                const row = doc.createElement("div");
                row.className = "battery-bike-row";
                appendTextElement(row, "span", "battery-bike-no", `車號 ${bike.bike_no}`);
                appendTextElement(row, "span", "battery-power", `${bike.battery_power}%`);
      ...
      async function runQuery() {
        ...
        try {
          ...
          let completed = 0;
          lastResults = await mapWithConcurrency(entries, 6, async entry => {
            ...
            const stationNo = String(matched.station_no || matched.sno || "").trim();
            let bikes = [];
            try { bikes = stationNo ? await getBatteryList(stationNo) : []; }
            catch (_) { return { ...entry, matched: false, bikes: [], fetchError: true }; }
            finally {
              completed += 1;
              setStatus(`已完成 ${completed}／${entries.length} 個場站……`);
            }
            return { ...entry, matched: true, stationNo, bikes };
          });
          ...
          setStatus(`更新時間 ${timeText}｜${lastResults.length - unmatchedCount} 個場站完成${unmatchedCount ? `｜${unmatchedCount} 個未配對` : ""}`);
        } catch (error) {
          const message = error?.name === "AbortError" ? "連線逾時，請檢查網路後重試。" : String(error?.message || error);
      ...
      thresholdInput.value = String(clampThreshold(preferences.threshold));
      const preferredScopes = Array.isArray(preferences.scopes) ? preferences.scopes : routes;
      for (const route of routes) {
        ...
        input.type = "checkbox";
        input.value = route;
        input.checked = preferredScopes.includes(route);
      ...
      if (!selectedRoutes().length && routes.length) {
        const firstInput = scopeList.querySelector("input");
      ...
      if (displayMode === "mobile" || win.matchMedia("(max-width: 700px)").matches) {
        fab.style.bottom = "calc(252px + env(safe-area-inset-bottom, 0px))";
      }
    })();
    </script>
    """
...
def configuration_family_key(sheet_name: str) -> str:
    """把工作表名稱中的 D1／D2／D3 視為同一組配置版本。"""
    return re.sub(r"d\s*[123]", "d#", str(sheet_name).lower())


def build_route_station_map(
    source: bytes | str,
    options: list[tuple[str, str]],
    selected_sheet: str,
    selected_shift: str,
) -> dict[str, list[dict]]:
    """依目前配置版本整理 D1～D3 場站，供電量範圍與緊急場站共用。"""
    selected_family = configuration_family_key(selected_sheet)
    route_station_map: dict[str, list[dict]] = {}

    for route in ("D1", "D2", "D3"):
        route_options = [
            (sheet_name, option_route)
            for sheet_name, option_route in options
            if str(option_route).upper() == route
        ]
        ...
        route_options.sort(
            key=lambda option: (
                configuration_family_key(option[0]) != selected_family,
                option[0] != selected_sheet,
                option[0],
            )
        )
        sheet_name, option_route = route_options[0]

        try:
            route_df = cached_parse_route(source, sheet_name, option_route, selected_shift)
        except Exception:
            continue
        if route_df.empty:
            continue
        ...
        for _, row in route_df.iterrows():
            ...
            station_key = normalize_youbike_station_key(station_name)
            if not station_key or station_key in seen_names:
                continue
            seen_names.add(station_key)
        ...
        if station_records:
            route_station_map[route] = station_records
...
mobile_input_mode = mobile_detected
...
route_station_map = build_route_station_map(
    active_base["bytes"],
    options,
    selected_sheet,
    selected_shift,
...
with st.expander("🌐 更新 YouBike 即時車數", expanded=True):
    ...
    if isinstance(previous_live_meta, dict) and previous_live_meta.get("fetched_at"):
        ...
        st.info(
            f"上次更新：{previous_live_meta['fetched_at']}"
            f"{source_time_text}｜{safe_nonnegative_int(previous_live_meta.get('matched_count'))} 個場站"
        )
            ...
            try:
                ...
                with st.spinner("正在更新場站車數……"):
                    ...
                    if live_summary["matched_count"] <= 0:
                        ...
                        previous_emergency_meta = metadata.get("__emergency__", {})
                        previous_emergency_stations = (
                            previous_emergency_meta.get("stations", [])
                            if isinstance(previous_emergency_meta, dict)
                            else []
                        )
                        emergency_stations = build_emergency_station_snapshot(
                            route_station_map,
                            live_payload["records"],
                            previous_emergency_stations,
                        )
                        metadata[current_context_key] = {
                        ...
                        metadata["__emergency__"] = {
                            "fetched_at": live_payload["fetched_at"],
                            "stations": emergency_stations,
                        }
                        save_cached_status(
                            active_base["token"],
                            active_base["expires_at"],
                        ...
                        st.success(
                            f"✅ 已更新 {live_summary['matched_count']}／"
                            f"{live_summary['total_count']} 個場站｜{live_payload['fetched_at']}"
                        )
                        if missing_count:
                            st.warning(
                                f"官網有 {missing_count} 個站號未回傳，本次未覆蓋其舊資料。"
                            )

                    if not live_report_df.empty:
                        problem_df = live_report_df[live_report_df["結果"] != "已寫入"]
                        with st.expander(
                            f"未更新明細：{len(problem_df)} 筆",
    ...
    st.caption("資料來自 YouBike 官網；官方資料可能有約 1 分鐘延遲。")

emergency_meta = status_cache.get("metadata", {}).get("__emergency__", {})
if isinstance(emergency_meta, dict) and emergency_meta.get("fetched_at"):
    emergency_stations = emergency_meta.get("stations", [])
    st.subheader("🚨 緊急場站計時")
    if emergency_stations:
        st.caption(
            f"{emergency_meta['fetched_at']} 更新｜每個無車或無空位場站獨立計時"
        )
        render_emergency_station_timers(emergency_stations)
    else:
        st.success("目前沒有無車或無空位場站。")
...
st.caption(f"{selected_route}｜{selected_shift}｜{selected_region}｜{len(working_df)} 個場站")
edited_df = coerce_nullable_current_status(working_df.copy())
...
if missing_bike_count or missing_ebike_count:
    st.warning(
        f"⚠️ 尚未同步：2.0 共 {missing_bike_count} 筆，"
        f"2.0E 共 {missing_ebike_count} 筆。"
    )
...
if result_df.empty:
    ...
    st.caption("CSV 與 Excel 會沿用目前排序。")

# 懸浮搜尋只讀取目前排序完成、實際顯示的分析結果。
render_floating_station_search(result_df, mobile_input_mode)
render_floating_battery_query(route_station_map, mobile_input_mode)
