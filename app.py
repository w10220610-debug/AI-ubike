...
APP_VERSION = "v26.1"
APP_VERSION_NAME = "電量查詢定位與行政區排序版"
...
def render_floating_battery_query(
    ...
    (() => {
      ...
      function buildDisplayGroups(threshold) {
        const decorated = lastResults.map(result => ({
          result,
        ...
        return Array.from(districtGroups.entries())
          .sort(([leftDistrict], [rightDistrict]) => leftDistrict.localeCompare(
            rightDistrict,
            "zh-Hant",
            { numeric: true, sensitivity: "base" },
          ))
      ...
      function renderStationCard(item, threshold, mode, isRecommended) {
        ...
        if (isRecommended) appendTextElement(meta, "span", "battery-recommended", "最近推薦");
      ...
      function renderResults() {
        ...
        const groups = buildDisplayGroups(threshold);
        for (const group of groups) {
          const title = group.mode === "nearest"
            ? `${group.title}｜${group.items.length} 個場站`
            : `${group.title}｜${group.total} 顆需更換`;
