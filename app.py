    ...
    components.html(
        ...
        (() => {{
            ...
            style.textContent = `
                ...
                #ubike-float-tools .uft-search {{ background: #ffbf00; font-size: 25px; }}
                ...
                #ubike-search-toast {{
                    position: fixed;
                    left: 50%;
                    bottom: var(--ubike-float-toast-bottom, 92px);
                    z-index: 2147483001;
                ...
                @media (max-width: 700px) {{
                    /* 預留 Streamlit Cloud「Manage app」與 iPhone 底部安全區高度。 */
                    #ubike-float-tools {{
                        right: 12px;
                        bottom: calc(112px + env(safe-area-inset-bottom, 0px));
                    }}
                    #ubike-float-tools .uft-button {{ width: 54px; height: 54px; }}
                    #ubike-float-tools .uft-panel {{
                        position: fixed;
                        left: 12px;
                        right: 12px;
            ...
            root.innerHTML = `
                <div class="uft-panel" hidden>
                    <div class="uft-title-row">
                        <div class="uft-title">🔎 快速搜尋場站</div>
                    ...
                    <div class="uft-hint">搜尋目前畫面中的場站｜快捷鍵 Ctrl + K</div>
                ...
                <div class="uft-actions">
                    ...
                    <button class="uft-button uft-search" type="button" title="快速搜尋場站（Ctrl + K）">🔎</button>
                </div>
            `;
            ...
            function detectBottomObstruction() {{
                ...
                /*
                 * Streamlit Cloud 的 Manage app 浮層可能會蓋在 App 上方。
                 * 若它位於同一份文件，就自動量出高度；若不在同一文件，
                 * updateFloatingPosition() 仍會套用手機版 112px 的安全預留。
                 */
                ...
                for (const element of candidates) {{
                    ...
                    if (rect.top < viewportHeight * 0.45 || rect.top >= viewportHeight) continue;
                    if (rect.bottom < viewportHeight - 10) continue;

                    obstructionHeight = Math.max(obstructionHeight, viewportHeight - rect.top);
                }}

                return Math.min(190, Math.max(0, obstructionHeight));
            }}

            function updateFloatingPosition() {{
                const mobile = isMobileLayout();
                const obstructionHeight = detectBottomObstruction();
                const bottomGap = mobile ? Math.max(112, obstructionHeight + 12) : 22;
            ...
            function scrollPageToTop() {{
                ...
                /* iOS Safari 偶爾會忽略第一次 smooth scroll，再補一次強制歸零。 */
                win.setTimeout(() => {{
                    for (const target of getScrollTargets()) {{
                        target.scrollTop = 0;
                        target.scrollLeft = 0;
            ...
            function ensureInputExpanderOpen(callback) {{
                const expanders = Array.from(doc.querySelectorAll('[data-testid="stExpander"]'));
                const target = expanders.find((node) =>
                    String(node.textContent || "").includes("輸入現場現況")
                );
                if (!target) {{
                    callback();
                    return;
                ...
                const collapsedButton = target.querySelector('[aria-expanded="false"]');
                if (details && !details.open) {{
                    const summary = details.querySelector("summary");
                    if (summary) summary.click();
                    win.setTimeout(callback, 320);
                    return;
                }}
                if (collapsedButton) {{
...
if result_df.empty:
    ...
    for region, region_df in result_df.groupby("行政區", sort=False):
        ...
        # 高度依資料列數完整展開，避免表格內再出現垂直捲動；排序功能仍照常套用。
        result_table_height = max(105, 40 + (len(display_df) * 35) + 4)
        st.dataframe(
            display_df,
            hide_index=True,
            use_container_width=True,
            height=result_table_height,
            row_height=35,
            column_config={
                "場站名稱": st.column_config.TextColumn("場站名稱", width="large"),
                "2.0 缺／多幾台": st.column_config.TextColumn("2.0 缺／多幾台", width="medium"),
                "2.0E 缺／多幾台": st.column_config.TextColumn("2.0E 缺／多幾台", width="medium"),
            },
        )
