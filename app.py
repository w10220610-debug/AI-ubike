...
def analyze_station_photos(uploaded_photos, station_df: pd.DataFrame) -> dict:
    ...
    for uploaded_photo in uploaded_photos:
        ...
        for row_items in rows:
            ...
            unit_count, bike_count, ebike_count = choose_station_counts(numbers, numeric_column_centers)
            ...
            # 重疊照片出現相同場站時，直接採用綜合信心較高的一列，不再跳確認視窗。
...
with st.expander("📷 多張照片直接更新現況", expanded=False):
    st.caption(
        ...
        "辨識成功的場站會直接寫入現況，未辨識或數字異常的場站維持原本數據。"
    )
        ...
        try:
            with st.spinner(f"正在辨識 {len(uploaded_station_photos)} 張照片並更新現況……"):
                ...
                if recognized_updates:
                    ...
                    st.success(
                        f"✅ 已直接更新 {len(recognized_updates)} 個場站。"
                        "照片未辨識到或數字不合理的場站均保留原值。"
                    )
                    ...
                    st.warning(
                        "這批照片沒有讀到可安全寫入的場站資料，因此所有現況數字都保持不變。"
                    )
        except ImportError:
            st.error(
