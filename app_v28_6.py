...
(() => {
  ...
  function startRecognition(){
    ...
    if(!recognition){
      recognition=new R(); recognition.lang="zh-TW"; recognition.continuous=!isIOS(); recognition.interimResults=false; recognition.maxAlternatives=5;
  ...
  function commandHelp(){
    speak("固定口令：賈維斯分析。賈維斯下一站。賈維斯抵達。賈維斯輸入上一般兩電。賈維斯完成。賈維斯跳過。賈維斯電池。賈維斯超低電。賈維斯重複。");
  }
  function handleTranscript(raw){
    ...
    if(hasWake) wakeWindowUntil=Date.now()+6500;
    const inWakeWindow=Date.now()<wakeWindowUntil;
    if(!hasWake && !inWakeWindow){ setIndicator(standbyLabel()); return; }
    const command=hasWake?stripWake(transcript):normalizeSpeechText(transcript);
    if(!command){ wakeWindowUntil=Date.now()+6500; speak("我在"); return; }
    ...
    if(explicitInput || (/[上下]/.test(actualCommand) && /般|電/.test(actualCommand))){
      ...
      speak(`${station}，${phrase}，已填入，請看畫面確認`, ()=>emit("fill_actual", {station_name:station, ...actual, heard:transcript}));
      return;
    }
    ...
    if(/^(?:清除輸入|清除输入|上下歸零|上下归零)$/.test(command)){
      ...
      speak(`${station}，上下車數量已歸零，請看畫面確認`,()=>emit("fill_actual",{station_name:station,...actual,heard:transcript})); return;
    }
    ...
    if(/我到了|已抵達|已抵达|抵達|抵达|到目的地|到站了|我到站|^到了$/.test(command)){
      if(!contextReady()){speak(contextReply());return;}
      const station=currentStation(); if(!station){speak("目前沒有目標場站");return;}
      if(text(args.mode)==="candidate") speak(`${station}，已抵達`, ()=>emit("lock_station",{station_name:station,heard:transcript}));
      else speak(`${station}，已抵達`);
      return;
    }
    if(/有人去|有人處理|有人处理|換下一站|换下一站|改往其他|跳過|跳过|不要去/.test(command)){
      if(!contextReady()){speak(contextReply());return;}
      emit("skip_current",{station_name:currentStation(),reason:/有人/.test(command)?"有人已前往":"使用者語音改站",heard:transcript}); return;
    }
    if(/^(?:改往|改去|換去|换去|前往|去)/.test(command)){
      if(!contextReady()){speak(contextReply());return;}
      const target=targetTextFromChange(command); emit("change_station",{station_name:currentStation(),target_text:target,heard:transcript}); return;
    }
...
def get_jarvis_voice_component():
    ...
    component_dir = Path(tempfile.gettempdir()) / "taitung_jarvis_voice_component_v4"
    component_dir.mkdir(parents=True, exist_ok=True)
    index_path = component_dir / "index.html"
