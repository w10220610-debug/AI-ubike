...
# Keep the durable learning pool behaviour used by V29 old.
            ...
            zone_items.append(
                {
                    **item,
                    ...
                    "official_name": str(
                        matched.get("station_name") or item["name"]
                    ).strip(),
...
def render_floating_server_battery(
    ...
    *,
    ...
    """V29 old battery page: continuous location, distance sort, district summary."""
...
(()=>{
 ...
 if(runtime.uiObserver){try{runtime.uiObserver.disconnect();}catch(_){} runtime.uiObserver=null;}
 if(runtime.uiInterval){try{win.clearInterval(runtime.uiInterval);}catch(_){} runtime.uiInterval=null;}
 let currentResults={};
 let running=false;
 ...
 function hasLocation(){return Number.isFinite(Number(runtime.locationState.lat))&&Number.isFinite(Number(runtime.locationState.lon));}
 ...
 function distanceFor(r){
   if(!hasLocation())return Infinity;
   return haversineMeters(runtime.locationState.lat,runtime.locationState.lon,r?.latitude,r?.longitude);
 }
 function distanceLabel(m){
   if(!Number.isFinite(m))return '';
   if(m<1000)return `約 ${Math.round(m)} 公尺`;
   const km=m/1000;
   return `約 ${km<10?km.toFixed(1):Math.round(km)} 公里`;
 }
 ...
 async function queryOne(spec,threshold,priority,force){
   ...
   if(cached&&!force&&now-cached.at<FRESH_MS){
     return {...cached.value,source:'memory_cache',age_seconds:(now-cached.at)/1000,
       latitude:validCoord(spec.latitude,-90,90),longitude:validCoord(spec.longitude,-180,180),
       requested_district:spec.district||spec.official_district||cached.value.requested_district,
       official_district:spec.official_district||cached.value.official_district};
   }
   let lastError=null;
   for(let attempt=0;attempt<2;attempt++){
     try{
       const data=await fetchJson(`${BATTERY_URL}?station_no=${encodeURIComponent(stationNo)}`);
       const bikes=normalizeBattery(data);
       ...
       const value={
         ...
         requested_district:spec.district||spec.official_district||'',
         official_district:spec.official_district||spec.district||'',
         station_name:spec.official_name||spec.name,
         station_no:stationNo,
         latitude:validCoord(spec.latitude,-90,90),
         longitude:validCoord(spec.longitude,-180,180),
         bikes,low_bikes:low,priority_bikes:pri,
         low_count:low.length,priority_count:pri.length,
         threshold,priority_threshold:priority,source:'live',age_seconds:0
       };
       runtime.cache.set(cacheKey,{at:Date.now(),value});
       return value;
     }catch(e){
       lastError=e;
   ...
   if(cached&&now-cached.at<STALE_MS){
     return {...cached.value,source:'stale_cache',age_seconds:(now-cached.at)/1000,error:String(lastError?.message||lastError),
       latitude:validCoord(spec.latitude,-90,90),longitude:validCoord(spec.longitude,-180,180),
       requested_district:spec.district||spec.official_district||cached.value.requested_district,
       official_district:spec.official_district||cached.value.official_district};
   }
 ...
 function resultRows(results,priorityEnabled,filter=''){
   const q=norm(filter);
   const rows=Object.values(results||{}).filter(r=>r&&!r.error&&num(r.low_count)>0&&(!q||norm(r.requested_name||r.station_name).includes(q)||norm(districtOf(r)).includes(q)));
   rows.sort((a,b)=>{
     if(hasLocation()){
   ...
   return rows.map((r,index)=>{
     ...
     const dist=distanceFor(r),distText=distanceLabel(dist);
     const nearest=index===0&&Number.isFinite(dist)?'<span class="nearest">最近推薦</span>':'';
     const subtitle=reverse
       ?`✅ 不用換 ${safe.length} 台｜${esc(districtOf(r))}${distText?`｜${distText}`:''}`
       :`${nearest}${esc(districtOf(r))}${distText?`｜${distText}`:''}｜需換 ${num(r.low_count)} 顆${urgent?`｜緊急 ${urgent}`:''}`;
     const badge=reverse?(safeMin===null?'反向':`最低 ${safeMin}%`):`需換 ${num(r.low_count)} 顆`;
 ...
 function districtSummaryHtml(results){
   const rows=Object.values(results||{}).filter(r=>r&&!r.error&&num(r.low_count)>0);
   const grouped=new Map();
   for(const r of rows){
     const district=districtOf(r);
     const cur=grouped.get(district)||{district,total:0,stations:0,minDistance:Infinity};
     cur.total+=num(r.low_count);
     cur.stations+=1;
     cur.minDistance=Math.min(cur.minDistance,distanceFor(r));
     grouped.set(district,cur);
   }
 ...
 function updateLocationUi(){
   const root=ensure(),el=root.querySelector('#ub-location-status');
   if(!el)return;
   const l=runtime.locationState;
   if(hasLocation()){
     const age=Math.max(0,Math.round((Date.now()-num(l.updatedAt,Date.now()))/1000));
     el.className='location-status ok';
     el.textContent=`定位完成（誤差約 ${Math.round(num(l.accuracy))} 公尺）｜依距離由近到遠排序｜${age} 秒前更新`;
   }else if(l.error){
     el.className='location-status bad';
     el.textContent=`定位未取得：${l.error}｜目前改用需更換顆數排序`;
   }else{
     el.className='location-status';
     el.textContent='正在取得定位…';
   }
 }
 function onLocation(position){
   const c=position?.coords||{};
   const lat=validCoord(c.latitude,-90,90),lon=validCoord(c.longitude,-180,180);
   if(lat===null||lon===null)return;
   runtime.locationState.lat=lat;
   runtime.locationState.lon=lon;
   runtime.locationState.accuracy=Math.max(0,num(c.accuracy,0));
   runtime.locationState.updatedAt=Date.now();
   runtime.locationState.error='';
   updateLocationUi();
   refreshViews();
 }
 function onLocationError(err){
   const messages={1:'定位權限未允許',2:'暫時無法取得位置',3:'定位逾時'};
   runtime.locationState.error=messages[num(err?.code)]||String(err?.message||'定位失敗');
   updateLocationUi();
   refreshViews();
 }
 function startLocationWatch(){
   const geo=win.navigator?.geolocation||navigator.geolocation;
   ...
   try{
     runtime.locationState.watchId=geo.watchPosition(onLocation,onLocationError,{enableHighAccuracy:true,maximumAge:15000,timeout:10000});
   }catch(e){
     runtime.locationState.error=String(e?.message||e||'定位啟動失敗');
     updateLocationUi();
   }
 }
...
.panel{background:rgba(12,28,43,.92);border:1px solid rgba(94,205,229,.23);border-radius:17px;padding:14px;margin:8px 0}.title-row,.actions,.summary{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.title-row{justify-content:space-between}
.mini{border:1px solid #294b63;background:#10283a;color:#bdeffc;border-radius:999px;padding:6px 10px;font-weight:750}.zones{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.zone{border:1px solid #2a4e66;border-radius:999px;padding:7px 10px;background:#0a1c2a;color:#d8f6ff}.zone input{accent-color:#4feaff}
.controls{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}.field{display:flex;flex-direction:column;gap:5px;color:#9db8c8;font-size:12px}.field input[type=number],#ub-v29-search{box-sizing:border-box;width:100%;min-height:42px;border:1px solid #2c526b;border-radius:11px;background:#081825;color:#eefcff;padding:8px 10px;font-size:16px}.toggle{display:flex;align-items:center;gap:8px;margin-top:10px}
.actions{margin-top:12px}.actions button{flex:1;min-width:125px;min-height:44px;border:0;border-radius:12px;font-size:15px;font-weight:900}.query{background:linear-gradient(90deg,#43e6ff,#74f3c5);color:#041018}.force{background:#38152c;color:#ffb8d6;border:1px solid #7e2d59!important}
.status{padding:10px 2px;color:#9dc1d1;font-size:13px}.updated{font-size:12px;color:#789aaa}.summary{margin:8px 0}.chip{border:1px solid #2b5269;border-radius:999px;padding:6px 9px;background:#0b2030;color:#bfefff;font-size:12px}.chip.hot{border-color:#8b2d5b;color:#ff9bc7}
.location-status{margin:9px 0;padding:10px 12px;border:1px solid #36546b;border-radius:12px;background:rgba(9,26,39,.9);color:#b7cbd7;font-size:13px}.location-status.ok{border-color:#287f69;color:#72f2bd}.location-status.bad{border-color:#8c3c60;color:#ff9bc2}
...
.station{background:rgba(11,26,40,.96);border:1px solid #24495e;border-radius:15px;margin:9px 0;overflow:hidden;transition:border-color .16s,box-shadow .16s,background .16s}.station summary{list-style:none;display:grid;grid-template-columns:minmax(0,1fr) 42px auto;align-items:center;gap:10px;padding:13px 14px}.station summary::-webkit-details-marker{display:none}.station-main{display:flex;min-width:0;flex-direction:column;gap:4px}.station-main strong{overflow-wrap:anywhere}.station summary small{color:#8db0c1}.nearest{display:inline-block;margin-right:6px;padding:2px 6px;border-radius:999px;background:#0b5d5c;color:#73ffed;font-size:11px;font-weight:900}
.reverse-toggle{width:40px;height:36px;padding:0;border:1px solid #315c72;border-radius:10px;background:#0d2637;color:#81eaff;font-size:21px;font-weight:900;line-height:1}.reverse-toggle.active{border-color:#55efb0;background:#0e4939;color:#b8ffe3}.min{min-width:74px;text-align:center;border-radius:999px;padding:7px 9px;background:#103447;color:#6cecff;font-weight:900;white-space:nowrap}.min.hot{background:#4d1734;color:#ff8cbd}
...
@media(max-width:560px){.controls{grid-template-columns:1fr}.shell{padding-left:10px;padding-right:10px}.bike{grid-template-columns:65px 1fr 48px}.station summary{grid-template-columns:minmax(0,1fr) 38px auto;gap:7px;padding:12px 10px}.reverse-toggle{width:36px;height:34px}.min{padding:7px}.district-card{min-height:82px;padding:10px}.district-card strong{font-size:25px}.district-head strong{font-size:17px}}
`; }
 function ensure(){
   let root=doc.getElementById(ROOT);
   if(!root){
     root=doc.createElement('div');root.id=ROOT;
     root.innerHTML=`<button id="ub-v29-fab" aria-label="電量查詢">⚡</button><section id="ub-v29-page" aria-hidden="true"><div class="shell"><header><button id="ub-v29-close">‹ 返回</button><div><h1>⚡ 2.0E 電量查詢｜測試版</h1><p>持續定位｜距離排序｜行政區統整</p></div></header><main id="ub-v29-main"></main></div></section>`;
   ...
   if(fab&&page&&!page.classList.contains('open')){fab.style.display='';fab.style.visibility='visible';fab.style.opacity='1';fab.style.pointerEvents='auto';fab.hidden=false;fab.removeAttribute('aria-hidden');}
   let style=doc.getElementById(ROOT+'-style');
   if(!style){style=doc.createElement('style');style.id=ROOT+'-style';doc.head.appendChild(style);}
   style.textContent=styleText();
   return root;
 }
 function repairFloatingButton(){
   let root=doc.getElementById(ROOT);
   if(!root||!root.isConnected){root=ensure();render();}
   const page=root?.querySelector('#ub-v29-page'),fab=root?.querySelector('#ub-v29-fab');
   if(!fab||!page)return;
   fab.onclick=open;
   const closeButton=root.querySelector('#ub-v29-close');if(closeButton)closeButton.onclick=close;
   if(!page.classList.contains('open')){fab.style.display='';fab.style.visibility='visible';fab.style.opacity='1';fab.style.pointerEvents='auto';fab.hidden=false;fab.removeAttribute('aria-hidden');}
 }
 function installSafeUiRepair(){
   runtime.safeUiRepair=repairFloatingButton;
   if(!runtime.safeUiInterval)runtime.safeUiInterval=win.setInterval(()=>{try{runtime.safeUiRepair?.();}catch(_){}},1500);
 }
 function open(){
   const root=ensure(),page=root.querySelector('#ub-v29-page');
   root.querySelector('#ub-v29-fab').style.display='none';
   page.classList.add('open');page.setAttribute('aria-hidden','false');
   root._htmlOverflow=doc.documentElement.style.overflow;root._bodyOverflow=doc.body.style.overflow;
   doc.documentElement.style.overflow='hidden';doc.body.style.overflow='hidden';
   render();startLocationWatch();
 }
 function close(){
   const root=ensure(),page=root.querySelector('#ub-v29-page');
   page.classList.remove('open');page.setAttribute('aria-hidden','true');
   root.querySelector('#ub-v29-fab').style.display='';
   doc.documentElement.style.overflow=root._htmlOverflow||'';doc.body.style.overflow=root._bodyOverflow||'';
   reverseStations.clear();
 }
 ...
 function refreshViews(filter){
   const root=ensure();
   const box=root.querySelector('#ub-results');
   if(box){
     const openKeys=new Set([...box.querySelectorAll('details.station[open]')].map(x=>x.getAttribute('data-station-key')||''));
     box.innerHTML=resultRows(currentResults,root.querySelector('#ub-pe')?.checked!==false,filter??root.querySelector('#ub-v29-search')?.value??'');
     for(const item of box.querySelectorAll('details.station'))if(openKeys.has(item.getAttribute('data-station-key')||''))item.open=true;
   }
   const district=root.querySelector('#ub-district-summary');
   if(district)district.innerHTML=districtSummaryHtml(currentResults);
   updateLocationUi();
 }
 function bindResultControls(root){
   ...
   box.onclick=e=>{
     ...
     refreshViews(root.querySelector('#ub-v29-search')?.value||'');
   };
 }
 function render(){
   ...
   const pr=(prefVersion>=2&&Number.isFinite(Number(p.priority_threshold)))?Number(p.priority_threshold):num(args.priority_threshold,69);
   ...
   root.querySelector('#ub-v29-search').oninput=e=>refreshViews(e.target.value);
   bindResultControls(root);updateLocationUi();
 }
 function summarize(done,total,failed){
   const vals=Object.values(currentResults).filter(r=>r&&!r.error);
   const lowStations=vals.filter(r=>num(r.low_count)>0).length;
   const lowBikes=vals.reduce((s,r)=>s+num(r.low_count),0);
   const urgent=vals.reduce((s,r)=>s+num(r.priority_count),0);
   const root=ensure();
   root.querySelector('#ub-summary').innerHTML=`<span class="chip">完成 ${done}/${total}</span><span class="chip">需換場站 ${lowStations}</span><span class="chip">需換 ${lowBikes} 顆</span>${urgent?`<span class="chip hot">緊急 ${urgent} 顆</span>`:''}${failed?`<span class="chip hot">失敗 ${failed}</span>`:''}`;
   refreshViews(root.querySelector('#ub-v29-search')?.value||'');
   root.querySelector('#ub-progress').style.width=`${total?Math.round(done/total*100):0}%`;
 }
 async function runQuery(force){
   if(running)return;
   const root=ensure(),map=args.route_station_map||{};
   const selected=[...root.querySelectorAll('[data-zone]:checked')].map(x=>x.value);
   const status=root.querySelector('#ub-status');
   if(!selected.length){status.textContent='請至少選擇一個範圍';return;}
   ...
   for(const z of selected)for(const item of (map[z]||[])){if(!item?.name||seen.has(item.name))continue;seen.add(item.name);specs.push(item);}
   currentResults={};running=true;runtime.run+=1;
   const runId=runtime.run;
   let index=0,done=0,failed=0;
   root.querySelectorAll('#ub-query,#ub-force').forEach(b=>b.disabled=true);
   status.textContent=`正在查詢 ${specs.length} 個場站…`;
   summarize(0,specs.length,0);
   async function worker(){
     while(index<specs.length&&runId===runtime.run){
       const spec=specs[index++];
       try{currentResults[spec.name]=await queryOne(spec,th,pr,force);}
       catch(e){failed++;currentResults[spec.name]={requested_name:spec.name,requested_district:spec.district||spec.official_district||'',official_district:spec.official_district||'',latitude:validCoord(spec.latitude,-90,90),longitude:validCoord(spec.longitude,-180,180),error:String(e?.message||e),low_count:0,priority_count:0};}
       finally{done++;status.textContent=`查詢中：${done}/${specs.length}｜失敗 ${failed}`;summarize(done,specs.length,failed);}
     }
   }
   await Promise.all(Array.from({length:Math.min(CONCURRENCY,specs.length)},worker));
   if(runId!==runtime.run)return;
   running=false;
   status.textContent=`查詢完成：${done-failed} 站成功${failed?`｜${failed} 站未取得`:''}`;
   root.querySelector('#ub-updated').textContent=`最後更新：${new Date().toLocaleString('zh-TW',{hour12:false})}`;
   root.querySelectorAll('#ub-query,#ub-force').forEach(b=>b.disabled=false);
   summarize(done,specs.length,failed);
 }
 ensure();render();startLocationWatch();installSafeUiRepair();repairFloatingButton();
})();
